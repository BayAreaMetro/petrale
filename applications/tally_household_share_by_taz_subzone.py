...
PARCEL_TO_SUBZONE_FILE = os.path.join(
    LARGE_DATASET_INPUT_DIR, "2018_10_17_parcel_to_taz1454sub.csv"
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "parcel_data_file",
        help='Parcel data file relative to "{}". e.g. "Blueprint Plus Crossing (s23)\\v1.5.5\\run998_parcel_data_2050.csv"'.format(
            URBANSIM_RUN_DIR
        ),
    )
    parser.add_argument("--parcel_gdb", help="Parcel gdb layer, optional for mapping")
    parser.add_argument(
        "--parcel_layer", help="Parcel layer in parcel_gdb, optional for mapping"
    )
    ...
    print(
        "Read {} rows from {}; head:\n{}".format(
            len(parcel_to_subzone_df),
            PARCEL_TO_SUBZONE_FILE,
            parcel_to_subzone_df.head(),
        )
    )
    # make PARCEL_ID and ZONE_ID an int rather than a float
    parcel_to_subzone_df["PARCEL_ID"] = (
        parcel_to_subzone_df["PARCEL_ID"].round(0).astype(numpy.int64)
    )
    parcel_to_subzone_df["ZONE_ID"] = (
        parcel_to_subzone_df["ZONE_ID"].round(0).astype(numpy.int64)
    )
    ...
    print(
        "Read {} rows from {}; head:\n{}".format(
            len(parcel_data_df), parcel_data_file, parcel_data_df.head()
        )
    )
    # filter to parcels with notnull(tothh) and tothh > 0
    parcel_data_df = parcel_data_df.loc[
        pandas.notnull(parcel_data_df["tothh"]) & (parcel_data_df["tothh"] > 0),
    ]
    ...
    taz_subzone_hh = (
        parcel_data_df.groupby(["ZONE_ID", "subzone"])
        .agg({"tothh": "sum"})
        .reset_index()
    )
    # pivot subzone to column
    taz_subzone_hh = taz_subzone_hh.pivot(
        index="ZONE_ID", columns="subzone", values="tothh"
    ).reset_index()
    ...
    # 3            4.0  398.0  0.0  0.0
    # 4            5.0  702.0  0.0  0.0
    taz_subzone_hh["tothh"] = (
        taz_subzone_hh["a"] + taz_subzone_hh["b"] + taz_subzone_hh["c"]
    )
    ...
    taz_subzone_hh[["TAZ", "SHRT", "LONG"]].to_csv(
        args.output_file, index=False, float_format="%.2f"
    )
    print("Wrote {} rows to {}".format(len(taz_subzone_hh), args.output_file))

    # save unpivoted for tableau
    taz_subzone_hh_unpivot = pandas.melt(
        taz_subzone_hh,
        id_vars=["TAZ"],
        value_vars=["SHRT", "LONG", "NONE"],
        value_name="share",
    )
    ...
    if not args.parcel_gdb or not args.parcel_layer or not args.output_gdb:
        print(
            "One or more of parcel_gdb, parcel_layer, output_gdb not specified -- skipping mapping"
        )
        sys.exit(0)
    ...
    if arcpy.Exists(args.parcel_layer):
        ...
        arcpy.CopyFeatures_management(
            os.path.join(args.parcel_gdb, args.parcel_layer),
    ...
    if arcpy.Exists(parcel_subzone):
        ...
        arcpy.TableToTable_conversion(
            PARCEL_TO_SUBZONE_FILE, arcpy.env.workspace, parcel_subzone
        )
    ...
    if arcpy.Exists(parcel_w_subzone):
        ...
        joined_layer = arcpy.AddJoin_management(
            args.parcel_layer, "PARCEL_ID", parcel_subzone, "PARCEL_ID", "KEEP_ALL"
        )
