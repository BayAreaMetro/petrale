...
if __name__ == "__main__":
    nets_df = pandas.read_csv(
        NETS_FILE,
        sep="\t",
        usecols=["DunsNumber", "Emp15", "HQDuns15", "FIPS15"],
        dtype={"DunsNumber": "str", "HQDuns15": "str", "FIPS15": "str"},
    )
    print(
        "Read {} rows from {}; head:\n{}".format(
            len(nets_df), NETS_FILE, nets_df.head()
        )
    )
    ...
    print(
        "Count of null    Emp15: {}".format(
            len(nets_df.loc[pandas.isnull(nets_df.Emp15)])
        )
    )
    print(
        "Count of null HQDuns15: {}".format(
            len(nets_df.loc[pandas.isnull(nets_df.HQDuns15)])
        )
    )

    # groupby HQDuns15
    nets_hq_df = nets_df.groupby(["HQDuns15"]).aggregate(
        {"DunsNumber": "count", "Emp15": "sum"}
    )
    nets_hq_df.rename(
        columns={"DunsNumber": "BayAreaEstCount", "Emp15": "BayAreaFirmEmp15"},
        inplace=True,
    )
