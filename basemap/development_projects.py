...
if os.getenv("USERNAME") == "lzorn":
    ...
    WORKING_DIR = (
        "C:\\Users\\MTCPB\\Documents\\ArcGIS\\Projects\\DevelopmentProjects\\2020 07 16"
    )
    LOG_FILE = os.path.join(WORKING_DIR, "devproj_{}.log".format(NOW))
...
cs1620 = os.path.join(
    SMELT_GDB, "cs1620"
)  # costar data  2016-2020, point feature class
cs1115 = os.path.join(
    SMELT_GDB, "cs1115"
)  # costar data  2011-2015, point feature class
...
rfcondo1115 = os.path.join(
    SMELT_GDB, "rf19_condounits1115"
)  # redfin condo data 2011-2015
rfother1115 = os.path.join(
    SMELT_GDB, "rf19_othertypes1115"
)  # redfin other data 2011-2015
...
manual_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_MANUAL_URL, "manual_portal"
)
oppsites_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_OPPSITE_URL, "oppsites_portal"
)
publicland_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_PUBLAND_URL, "publicland_portal"
)
malloffice_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_MALLOFFICE_URL, "malloffice_portal"
)
growth_geo_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_GROWTHGEO_URL, "growth_geo_portal"
)
mallpub_alt2_portal = arcpy.MakeFeatureLayer_management(
    MTC_ONLINE_MALLPUB_ALT2_URL, "mallpub_alt2_portal"
)
...
arcpy.FeatureClassToFeatureClass_conversion(
    oppsites_portal, SMELT_GDB, "opportunity_dp"
)
arcpy.FeatureClassToFeatureClass_conversion(publicland_portal, SMELT_GDB, "pubsites_dp")
arcpy.FeatureClassToFeatureClass_conversion(
    malloffice_portal, SMELT_GDB, "mallsites_dp"
)
arcpy.FeatureClassToFeatureClass_conversion(growth_geo_portal, SMELT_GDB, "growth_geo")
arcpy.FeatureClassToFeatureClass_conversion(
    mallpub_alt2_portal, SMELT_GDB, "mallpub_alt2"
)

GGtra = arcpy.SelectLayerByAttribute_management(
    growth_geo,
    "NEW_SELECTION",
    "designatio <> 'High-Resource Area (16-30 min bus)' And designatio <> 'Priority Production Area'",
)
...
if __name__ == "__main__":
    ...
    for dataset in arcpy.ListDatasets():
        ...
        logger.info(
            "    feature classes: {} ".format(
                arcpy.ListFeatureClasses(feature_dataset=dataset)
            )
        )
    ...
    logger.info(
        "Feature Class {} has {} records with incl = 1".format(manual_dp, countOne)
    )
    ...
    try:
        ...
        if int(count[0]) > 100:
            logger.info(
                "Found layer {} with {} rows -- skipping creation".format(
                    joinFN, int(count[0])
                )
            )
        ...
        ### 1 SPATIAL JOINS
        logger.info(
            "Creating layer {} by spatial joining manual pipeline data ({}) and parcels ({})".format(
                joinFN, manual_dp, p10_pba50
            )
        )
        ...
        arcpy.AlterField_management(
            joinFN, "Average_Weighted_Rent", "m_average_weighted_rent"
        )
        arcpy.AlterField_management(joinFN, "x", "p_x")
        arcpy.AlterField_management(joinFN, "y", "p_y")
        arcpy.AlterField_management(
            joinFN, "geom_id", "p_geom_id"
        )  # this is from the parcel file
        ...
        arcpy.AddField_management(joinFN, "scen29", "SHORT")
        ...
        arcpy.CalculateField_management(joinFN, "scen28", 1)
        arcpy.CalculateField_management(
            joinFN, "scen29", 1
        )  # these are committed so 1 for all scens
        # not sure how to change zip field type
        # arcpy.CalculateField_management(joinFN, "zip", '!m_zip!')
        arcpy.CalculateField_management(
            joinFN, "x", "!X_1!"
        )  # use spatial info from parcel file
        arcpy.CalculateField_management(
            joinFN, "y", "!Y_1!"
        )  # use spatial info from parcel file
        arcpy.CalculateField_management(
        ...
        arcpy.CalculateField_management(
            joinFN, "residential_units", "!m_residential_units!"
        )
        ...
        arcpy.CalculateField_management(
            joinFN, "last_sale_year", "!m_sale_date!"
        )  # need to make into year
        arcpy.CalculateField_management(
            joinFN, "last_sale_price", "!m_last_sale_price!"
        )
        ...
        if countTwo == countOne:
            logger.info(
                "All records with incl = 1 in feature class {} are included in the temp file".format(
                    manual_dp
                )
            )
        else:
        ...
        logger.info(
            "{} list has {} records with geom_id info missing".format(joinFN, nullcount)
        )
    ...
    for fc in cs:
        ...
        logger.info(
            "Feature Class {} has {} records with incl = 1".format(fc, countOne)
        )
        ...
        ### 1 SPATIAL JOINS
        logger.info(
            "Creating layer {} by spatial joining costar ({}) and parcels ({})".format(
                joinFN, fc, p10_pba50
            )
        )
        ...
        arcpy.AlterField_management(
            joinFN, "Average_Weighted_Rent", "cs_average_weighted_rent"
        )
        arcpy.AlterField_management(joinFN, "x", "p_x")
        arcpy.AlterField_management(joinFN, "y", "p_y")
        arcpy.AlterField_management(
            joinFN, "geom_id", "p_geom_id"
        )  # this is from the parcel
        ...
        arcpy.AddField_management(
            joinFN, "zip", "TEXT", "", "", 50
        )  ## this is changed from LONG to TEXT because cs1115 file has some text formatted zipcode with "-"
        ...
        arcpy.CalculateField_management(
            joinFN, "building_sqft", "!Rentable_Building_Area!"
        )  # how often null for res
        arcpy.CalculateField_management(
            joinFN, "non_residential_sqft", "!Rentable_Building_Area!"
        )  # need to zero out for res
        arcpy.CalculateField_management(
            joinFN, "residential_units", "!Number_Of_Units!"
        )
        arcpy.CalculateField_management(joinFN, "unit_ave_sqft", "!Avg_Unit_SF!")
        arcpy.CalculateField_management(joinFN, "tenure", "'Rent'")
        arcpy.CalculateField_management(
            joinFN, "rent_type", "!cs_rent_type!"
        )  # need to clean
        ...
        with arcpy.da.UpdateCursor(
            joinFN, ["Number_Of_Parking_Spaces", "parking_spaces"]
        ) as cursor:
            for row in cursor:
        ...
        # arcpy.CalculateField_management(joinFN, "rent_ave_unit", )
        arcpy.CalculateField_management(
            joinFN, "last_sale_year", "!cs_last_sale_date!"
        )  # need to make into year
        arcpy.CalculateField_management(
            joinFN, "last_sale_price", "!cs_last_sale_price!"
        )

        with arcpy.da.UpdateCursor(
            joinFN, ["cs_rent_type", "residential_units", "deed_restricted_units"]
        ) as cursor:
            for row in cursor:
                if row[0] == "Affordable":
                    row[2] = row[1]
        ...
        if countTwo == countOne:
            logger.info(
                "All records with incl = 1 in feature class {} is included in the temp file".format(
                    fc
                )
            )
        else:
        ...
        with arcpy.da.UpdateCursor(
            joinFN, ["building_type", "non_residential_sqft"]
        ) as cursor:
            for row in cursor:
                if row[0] == "HT":
                    row[1] = 0
    ...
    logger.info(
        "Feature Class {} has {} records with incl = 1".format(basis_pipeline, countOne)
    )
    ...
    ### 1 SPATIAL JOINS
    logger.info(
        "Creating layer {} by spatial joining BASIS pipeline data ({}) and parcels ({})".format(
            joinFN, basis_pipeline, p10_pba50
        )
    )
    ...
    arcpy.CalculateField_management(
        joinFN, "building_sqft", "!building_sqft!"
    )  # how often null for res
    arcpy.CalculateField_management(
        joinFN, "residential_units", "!p_residential_units!"
    )
    ...
    logger.info(
        "Feature Class {} has {} records with incl = 1".format(basis_pb_new, countOne)
    )
    joinFN = "ttt_basis_pb_new_p10__pba50"
    dev_projects_temp_layers.append(joinFN)

    logger.info(
        "Creating layer {} by spatial joining basis pba pipeline data ({}) and parcels ({})".format(
            joinFN, basis_pb_new, p10_pba50
        )
    )
    arcpy.DeleteField_management(
        basis_pb_new, "geom_id"
    )  # this column is causing trouble
    ...
    nonZero = arcpy.SelectLayerByAttribute_management(
        "b10_unitSUM", "NEW_SELECTION", '"SUM_residential_units" > 0'
    )  # choose only parcels with residential units already
    ...
    arcpy.AddJoin_management(
        "basis_join", "PARCEL_ID", "nonZeroParcel", "parcel_id", "KEEP_COMMON"
    )
    ...
    with arcpy.da.SearchCursor(
        "basis_join",
        [
            "ttt_basis_pb_new_p10__pba50.parcel_id",
            "ttt_basis_pb_new_p10__pba50.residential_units",
            "nonZeroParcel.SUM_residential_units",
        ],
    ) as cursor:
        for row in cursor:
    ...
    logger.info(
        "There are {} records in basis_pb_new that do not see increase in residential unit counts on the parcel".format(
            len(parcelRemoveList)
        )
    )
    # remove join
    ...
    with arcpy.da.UpdateCursor(
        joinFN, ["PARCEL_ID", "scen20", "scen24", "scen25"]
    ) as cursor:
        for row in cursor:
    ...
    arcpy.CalculateField_management(joinFN, "scen29", "!scen25!")
    ...
    with arcpy.da.UpdateCursor(
        joinFN, ["urbansim_parcels_v3_geo_county", "county"]
    ) as cursor:
        for row in cursor:
            if row[0] == 1:
                row[1] = "Alameda"
    ...
    arcpy.CalculateField_management(
        joinFN, "residential_units", "!n_residential_units!"
    )

    with arcpy.da.UpdateCursor(
        joinFN, ["building_sqft", "residential_units", "unit_ave_sqft"]
    ) as cursor:
        for row in cursor:
            if row[1] is int:
                row[2] = row[0] / row[1]
    ...
    arcpy.CalculateField_management(
        joinFN, "last_sale_year", "!last_sale_date!"
    )  # need to make into year
    arcpy.CalculateField_management(joinFN, "deed_restricted_units", 0)
    ...
    for fc in rf:
        ...
        ### 1 SPATIAL JOINS
        logger.info(
            "Creating layer {} by spatial joining redfin ({}) and parcels ({})".format(
                joinFN, fc, p10_pba50
            )
        )
        ...
        arcpy.AlterField_management(
            joinFN, "x", "p_x"
        )  # this is from the parcel centroid
        arcpy.AlterField_management(
            joinFN, "y", "p_y"
        )  # this is from the parcel centroid
        arcpy.AlterField_management(
        ...
        arcpy.CalculateField_management(
            joinFN, "building_sqft", "!SQFT!"
        )  # how often null for res
        arcpy.CalculateField_management(
            joinFN, "non_residential_sqft", 0
        )  # seems redfin data are all residential
        ...
        with arcpy.da.UpdateCursor(
            joinFN, ["SQFT", "UNITS", "unit_ave_sqft"]
        ) as cursor:
            for row in cursor:
                row[2] = row[0] / row[1]
                cursor.updateRow(row)
        arcpy.CalculateField_management(
            joinFN, "tenure", "'Sale'"
        )  # is redfin data rental?
        arcpy.CalculateField_management(
            joinFN, "last_sale_year", "!SOLD_DATE!"
        )  # need to make into year
    ...
    logger.info(
        "Merging feature classes {} into {}".format(
            dev_projects_temp_layers, pipeline_fc
        )
    )
    ...
    try:
        ...
        # go ahead and create it
        logger.info(
            "Creating layer {} by spatial joining pub sites data ({}) and parcels ({})".format(
                joinFN, pub_sites, p10_pba50
            )
        )
        ...
        arcpy.CalculateField_management(
            joinFN, "development_projects_id", "!developmen!"
        )
        ...
        pub_GGtra = arcpy.SelectLayerByLocation_management(
            joinFN, "WITHIN", GGtra, None, "NEW_SELECTION", "NOT_INVERT"
        )
    ...
    try:
        ...
        # go ahead and create it
        logger.info(
            "Creating layer {} by spatial joining mall office sites data ({}) and parcels ({})".format(
                joinFN, mall_sites, p10_pba50
            )
        )
    ...
    try:
        ...
        # go ahead and create it
        logger.info(
            "Creating layer {} by spatial joining opp sites data ({}) and parcels ({})".format(
                joinFN, opp_sites, p10_pba50
            )
        )
        ...
        arcpy.AlterField_management(
            joinFN, "non_residential_sqft", "o_non_residential_sqft"
        )
        ...
        arcpy.AlterField_management(
            joinFN, "Average_Weighted_Rent", "average_weighted_rent"
        )
        ...
        arcpy.CalculateField_management(
            joinFN, "residential_units", "!o_residential_units!"
        )
        ...
        with arcpy.da.UpdateCursor(joinFN, "type") as cursor:
            for row in cursor:
                if row[0] == "pb50_opp":
                    cursor.deleteRow()
        with arcpy.da.UpdateCursor(
            joinFN, ["source", "building_name", "o_source"]
        ) as cursor:
            for row in cursor:
                if row[1] == "incubator":
                    row[0] = row[1]
    ...
    logger.info(
        "Merging feature classes {} into {}".format(
            dev_projects_temp_layers, devproj_fc
        )
    )
    ...
    logger.info(
        "Pipeline list has {} records with building type info missing".format(nullcount)
    )
    ...
    arcpy.SelectLayerByAttribute_management(
        pipeline_fc, "NEW_SELECTION", '"residential_units">0'
    )
    arcpy.SelectLayerByAttribute_management(
        pipeline_fc, "SUBSET_SELECTION", '"non_residential_sqft" = "temp_building_sqft"'
    )
    arcpy.SelectLayerByAttribute_management(
        pipeline_fc, "SUBSET_SELECTION", '"building_type_id" = 3'
    )  # HM
    arcpy.CalculateField_management(
        pipeline_fc,
        "building_sqft",
        "!residential_units! * 1400 + !temp_building_sqft! ",
        "PYTHON",
    )
    ...
    arcpy.SelectLayerByAttribute_management(
        pipeline_fc, "SUBSET_SELECTION", '"building_type_id" = 12'
    )  # MR
    arcpy.CalculateField_management(
        pipeline_fc,
        "building_sqft",
    ...
    arcpy.SelectLayerByAttribute_management(
        pipeline_fc, "NEW_SELECTION", '"building_sqft" is NULL '
    )
    arcpy.CalculateField_management(
        pipeline_fc, "building_sqft", "!temp_building_sqft!", "PYTHON"
    )
    ...
    arcpy.AlterField_management(
        devproj_fc, "building_type", "building_type_det", "building_type_det"
    )
    ...
    nullcount = arcpy.GetCount_management(btnull)
    logger.info(
        "Development Project list has {} records with building type info missing".format(
            nullcount
        )
    )
    arcpy.Delete_management(btnull)
    ...
    arcpy.SelectLayerByAttribute_management(
        devproj_fc, "NEW_SELECTION", '"residential_units">0'
    )
    arcpy.SelectLayerByAttribute_management(
        devproj_fc, "SUBSET_SELECTION", '"non_residential_sqft" = "temp_building_sqft"'
    )
    arcpy.SelectLayerByAttribute_management(
        devproj_fc, "SUBSET_SELECTION", '"building_type_id" = 3'
    )  # HM
    arcpy.CalculateField_management(
        devproj_fc,
        "building_sqft",
    ...
    arcpy.SelectLayerByAttribute_management(
        devproj_fc, "SUBSET_SELECTION", '"building_type_id" = 12'
    )  # MR
    arcpy.CalculateField_management(
        devproj_fc,
        "building_sqft",
    ...
    arcpy.SelectLayerByAttribute_management(
        devproj_fc, "NEW_SELECTION", '"building_sqft" is NULL '
    )
    arcpy.CalculateField_management(
        devproj_fc, "building_sqft", "!temp_building_sqft!", "PYTHON"
    )
    ...
    arcpy.Statistics_analysis(
        devproj_fc, "res_stats_y", [["residential_units", "SUM"]], "year_built"
    )
    ...
    logger.info(
        "Total number of residential units in {} file: {:,} units".format(
            devproj_fc, int(sum_value)
        )
    )

    # number of nonres sqft by year
    arcpy.Statistics_analysis(
        devproj_fc, "nonres_stats_y", [["non_residential_sqft", "SUM"]], "year_built"
    )
    # then calculate the total
    arcpy.Statistics_analysis(
        devproj_fc, "nonres_stats_a", [["non_residential_sqft", "SUM"]]
    )
    # get the total result and write into log
    ...
    logger.info(
        "Total number of non residential square footage in {}: {:,} square feet".format(
            devproj_fc, int(sum_value)
        )
    )
    ...
    arcpy.Statistics_analysis(
        pipeline_fc, p_pipeline, [["development_projects_id", "COUNT"]], "geom_id"
    )
    ...
    arcpy.MakeTableView_management(
        p_pipeline, ppCount, "COUNT_development_projects_id > 1"
    )
    countParcelP = arcpy.GetCount_management(ppCount)
    logger.info(
        "There are {} of parcels with multiple project points (more than 1) on them in the pipeline file".format(
            countParcelP
        )
    )
    ...
    p_dev = "p_dev"
    arcpy.Statistics_analysis(
        devproj_fc, p_dev, [["development_projects_id", "COUNT"]], "geom_id"
    )
    # there are projects with geom_id null, so in order to count, delete those first
    ...
    logger.info(
        "There are {} of parcels with multiple project points (more than 1) on them".format(
            countParcelD
        )
    )
    ...
    i = 1
    with arcpy.da.UpdateCursor(
        devproj_fc_reordered, "development_projects_id"
    ) as cursor:
        for row in cursor:
            if i <= int(count[0]):
                row[0] = i
    ...
    arcpy.TableToTable_conversion(
        devproj_fc_reordered, WORKING_DIR, development_project_output
    )
    logger.info(
        "Wrote {}".format(os.path.join(WORKING_DIR, development_project_output))
    )
    ...
    pipeline_df = pipeline_df.rename(
        columns={
            "development_proj": "development_projects_id",
    ...
    development_project_df = pd.read_csv(
        os.path.join(WORKING_DIR, development_project_output)
    )
    development_project_df = development_project_df.rename(
        columns={
            "development_proj": "development_projects_id",
            "non_residential_": "non_residential_sqft",
            "development_type": "development_type_id",
    ...
    for key, value in field_types.items():
        ...
        if key == "geom_id" or key == "PARCEL_ID":
            ...
            development_project_df[key] = (
                development_project_df[key].round(0).astype(value)
            )
        else:
    ...
    pipeline_df.loc[
        (pipeline_df["residential_units"] < 0)
        & (pipeline_df.building_type.isin(res_type)),
        "residential_units",
    ] = 0
    pipeline_df.loc[
        (pipeline_df["residential_units"] != 0)
        & (pipeline_df.building_type.isin(nonres_type)),
        "residential_units",
    ] = 0

    development_project_df.loc[
        (development_project_df["residential_units"] < 0)
        & (development_project_df.building_type.isin(res_type)),
        "residential_units",
    ] = 0
    development_project_df.loc[
        (development_project_df["residential_units"] != 0)
        & (development_project_df.building_type.isin(nonres_type)),
        "residential_units",
    ] = 0

    pipeline_df.to_csv(os.path.join(WORKING_DIR, pipeline_output), index=False)
    development_project_df.to_csv(
        os.path.join(WORKING_DIR, development_project_output), index=False
    )
    ...
    for fc in fcs:
        arcpy.FeatureClassToFeatureClass_conversion(
            fc, os.path.join(WORKING_DIR, out_name), arcpy.Describe(fc).name
        )
    ...
    arcpy.FeatureClassToFeatureClass_conversion(
        ...
        arcpy.env.workspace,
        "p1115",
        "year_built >= 2011 AND year_built <= 2015",
    )
    p1115 = "p1115"
    ...
    arcpy.FeatureClassToFeatureClass_conversion(
        p1115, arcpy.env.workspace, "p1115_add", "action = 'add'"
    )
    arcpy.FeatureClassToFeatureClass_conversion(
        p1115, arcpy.env.workspace, "p1115_build", "action = 'build'"
    )
    ...
    arcpy.DeleteField_management(
        p1115_build, fields2Delete
    )  # because the two dataset should have the same structure
    ...
    with arcpy.da.UpdateCursor(
        b10, ["development_type_id", "building_type", "building_type_id"]
    ) as cursor:
        for row in cursor:
            if row[0] == 1:
                row[1] = "HS"
    ...
    arcpy.TableToTable_conversion(
        b10_p1115_part1, arcpy.env.workspace, "b10_p1115_part1_copy"
    )

    # part 2: remove and merge
    parcelBuildList = [
        row[0] for row in arcpy.da.SearchCursor(p1115_build, "parcel_id")
    ]
    ...
    rawp10_b15_pba50 = "rawp10_b15_pba50_{}".format(NOW)[
        0:26
    ]  # delete ".time" part, because that dot breaks it.
    ...
    nullcount = arcpy.GetCount_management(btnull)
    logger.info(
        "Building file list has {} records with building type info missing".format(
            nullcount
        )
    )
    arcpy.Delete_management(btnull)
    ...
    arcpy.Statistics_analysis(
        b10_p1115_part1_copy, "removed_units", [["residential_units", "SUM"]]
    )
    ...
    arcpy.Statistics_analysis(
        b10_p1115_part1_copy, "removed_nonres", [["non_residential_sqft", "SUM"]]
    )
    ...
    arcpy.Statistics_analysis(
        p1115_build, "built_units", [["residential_units", "SUM"]]
    )
    ...
    arcpy.Statistics_analysis(
        p1115_build, "built_nonres", [["non_residential_sqft", "SUM"]]
    )
    ...
    if sum_value1 >= sum_value3:
        logger.info(
            "There is a net decrease of {} units from {} units to {} units after incorporating the 'built' projects".format(
                sum_value1 - sum_value3, sum_value1, sum_value3
            )
        )
    else:
        logger.info(
            "There is a net increase of {} units from {} units to {} units after incorporating the 'built' projects".format(
                sum_value3 - sum_value1, sum_value1, sum_value3
            )
        )
    if sum_value2 >= sum_value4:
        logger.info(
            "There is a net decrease of {} square feet of nonresidential from {} sqft to {} sqft  after incorporating the 'built' projects".format(
                sum_value2 - sum_value4, sum_value2, sum_value4
            )
        )
    else:
        logger.info(
            "There is a net increase of {} square feet of nonresidential from {} sqft  to {} sqft  after incorporating the 'built' projects".format(
                sum_value4 - sum_value2, sum_value2, sum_value4
            )
        )
