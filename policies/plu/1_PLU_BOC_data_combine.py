...
if os.getenv("USERNAME") == "ywang":
    ...
    GITHUB_URBANSIM_DIR = (
        "C:\\Users\\{}\\Documents\\GitHub\\bayarea_urbansim\\data".format(
            os.getenv("USERNAME")
        )
    )
...
PBA40_ZONING_BOX_DIR = os.path.join(
    M_URBANSIM_DIR, "Horizon", "Large General Input Data"
)
PBA50_ZONINGMOD_DIR = os.path.join(
    M_URBANSIM_DIR, "Final_Blueprint", "Zoning Modifications"
)
OTHER_INPUTS_DIR = os.path.join(
    M_URBANSIM_DIR, "Final_Blueprint", "Base zoning", "input"
)
...
def impute_max_dua(df_original, boc_source):
    ...
    logger.info(
        "impute_max_dua_{}: Before imputation, number of parcels with missing max_dua_{}: {:,}".format(
    ...
    df["max_dua_from_far"] = (
        ...
        * dev_capacity_calculation_module.SQUARE_FEET_PER_ACRE
        / dev_capacity_calculation_module.SQUARE_FEET_PER_DU
    )
    df["max_far_from_height"] = (
        df["max_height_" + boc_source]
        / dev_capacity_calculation_module.FEET_PER_STORY
        * dev_capacity_calculation_module.PARCEL_USE_EFFICIENCY
    )
    ...
    df.loc[
        (df["max_dua_" + boc_source].notnull()) & (df["max_dua_" + boc_source] > 0),
    ...
    df.loc[
        ...
        & (df.max_dua_from_far > 0),
        "source_dua_" + boc_source,
    ] = "imputed from max_far (as min)"
    ...
    df.loc[
        ...
        "source_dua_" + boc_source,
    ] = "imputed from max_height (as min)"
    ...
    df.loc[
        (df["source_dua_" + boc_source] == "missing")
        & df.max_dua_from_height.notnull()
        & df.max_dua_from_far.isnull(),
        "source_dua_" + boc_source,
    ] = "imputed from max_height"

    # imputation is decided -- set it
    df.loc[
        df["source_dua_" + boc_source] == "imputed from max_height (as min)",
        "max_dua_" + boc_source,
    ] = df.loc[
        df["source_dua_" + boc_source] == "imputed from max_height (as min)",
        "max_dua_from_height",
    ...
    df.loc[
        df["source_dua_" + boc_source] == "imputed from max_height",
        "max_dua_" + boc_source,
    ] = df.loc[
        df["source_dua_" + boc_source] == "imputed from max_height",
        "max_dua_from_height",
    ...
    df.loc[
        df["source_dua_" + boc_source] == "imputed from max_far (as min)",
        "max_dua_" + boc_source,
    ] = df.loc[
        df["source_dua_" + boc_source] == "imputed from max_far (as min)",
        "max_dua_from_far",
    ...
    df.loc[
        df["source_dua_" + boc_source] == "imputed from max_far",
        "max_dua_" + boc_source,
    ] = df.loc[
        df["source_dua_" + boc_source] == "imputed from max_far", "max_dua_from_far"
    ]
...
def impute_max_far(df_original, boc_source):
    ...
    logger.info(
        "impute_max_far_{}: Before imputation, number of parcels with missing max_far_{}: {:,}".format(
    ...
    # this is set already -- nothing to do
    df.loc[
        (df["max_far_" + boc_source].notnull()) & (df["max_far_" + boc_source] > 0),
    ...
    df.loc[
        (df["source_far_" + boc_source] == "missing")
        & df.max_far_from_height.notnull(),
        "source_far_" + boc_source,
    ] = "imputed from max_height"

    # imputation is decided -- set it
    df.loc[
        df["source_far_" + boc_source] == "imputed from max_height",
        "max_far_" + boc_source,
    ] = df.loc[
        df["source_far_" + boc_source] == "imputed from max_height",
        "max_far_from_height",
...
def impute_basis_devtypes_from_pba40(df):
    ...
    for btype in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES:
        ...
        df.loc[
            df[btype + "_basis"].isnull(), "source_" + btype + "_basis"
        ] = "missing"  # or missing if null

        logger.info(
            "Before imputation of {}_basis:\n{}".format(
                btype, df["source_" + btype + "_basis"].value_counts()
            )
        )
        ...
        # => impute
        impute_idx = (df[btype + "_basis"].isnull()) & (df[btype + "_pba40"].notnull())
...
def impute_basis_max_height_from_pba40(df):
    ...
    df.loc[
        df["max_height_basis"].isnull(), "source_height_basis"
    ] = "missing"  # or missing if null
    ...
    #    and the pba40 value is present
    # => impute
    impute_idx = (df["max_height_basis"].isnull()) & (df["max_height_pba40"].notnull())
    # impute and note source
...
if __name__ == "__main__":
    ...
    p10_pba40_pz.rename(
        columns={"nodev": "nodev_pba40", "zoning_id": "zoning_id_pba40"}, inplace=True
    )
    ...
    logger.info(
        "Out of {0:,} p10 parcels, {1:,} or {2:.1f}% are missing 'zoning_id' values".format(
            len(p10_pba40_pz),
            len(p10_pba40_pz_missing),
            100.0 * len(p10_pba40_pz_missing) / len(p10_pba40_pz),
        )
    )
    ...
    pba40_plu["jz_o"] = pba40_plu["city_pba40"].str.cat(
        pba40_plu["name_pba40"], sep=" "
    )
    logger.info(
        "Out of {:,} rows in pba40_plu, {:,} have unique values of 'id', {:,} have unique values of 'jz_o'".format(
            len(pba40_plu),
            len(pba40_plu.id_pba40.unique()),
            len(pba40_plu.jz_o.unique()),
        )
    )

    # using the zoning_id, get the pba40 zoning data (intensities, allowed building types)
    p10_pba40_plu = pd.merge(
        left=p10_pba40_pz,
        right=pba40_plu,
    ...
    logger.info(
        "Out of {0:,} rows in p10_pba40_plu, {1:,} or {2:.1f}% are missing 'jz_o' values".format(
            len(p10_pba40_plu),
            len(p10_pba40_plu_missing),
            100.0 * len(p10_pba40_plu_missing) / len(p10_pba40_plu),
        )
    )
    ...
    basis_parcel_plu_id_file = os.path.join(
        OTHER_INPUTS_DIR, "p10_urbansim_boc_opt_b_v2.csv"
    )

    basis_parcel_plu_id = pd.read_csv(
        basis_parcel_plu_id_file,
        usecols=["parcel_id", "plu_id"],
        dtype={"parcel_id": float},
    )
    logger.info(
        "Read {:,} rows from {}, with header: \n{}".format(
    ...
    basis_parcel_plu_id = basis_parcel_plu_id.loc[
        basis_parcel_plu_id.parcel_id.notnull()
    ]
    # convert parcel_id to integer
    basis_parcel_plu_id["parcel_id"] = basis_parcel_plu_id["parcel_id"].apply(
        lambda x: int(round(x))
    )

    logger.info(
        "After dropping nan parcel_id, BASIS parcel-plu_id has {} parcels, {} unique parcel_id".format(
    ...
    if len(basis_parcel_plu_id) == basis_parcel_plu_id_rows:
        logger.info(
            "No duplicate parcel_ids found in {}".format(basis_parcel_plu_id_file)
        )
    else:
        logger.warning(
            "Dropped duplicate parcel_id rows from {}".format(basis_parcel_plu_id_file)
        )
        logger.warning(
            "Went from {:,} to {:,} rows; dropped {:,} duplicates".format(
    ...
    basis_boc_lookup_file = os.path.join(
        OTHER_INPUTS_DIR, "boc_lookup_2020_rev_10_13_20_final.csv"
    )
    ...
    basis_boc_lookup = pd.read_csv(
        basis_boc_lookup_file, usecols=basis_boc_lookup_columns
    )
    logger.info(
        "Read {:,} rows from {} with {} unique zoning_id, with header: \n{}".format(
    ...
    logger.info(
        "After dropping duplicates, {:,} rows left, with {} unique zoning_id".format(
            len(basis_boc_lookup), len(basis_boc_lookup.zoning_id.unique())
        )
    )

    # rename some columns to be consistent with the data's previous versions
    basis_boc_lookup.rename(
        columns={
            "jurisdiction": "plu_jurisdiction",
    ...
    basis_boc = basis_parcel_plu_id.merge(
        basis_boc_lookup, left_on="plu_id", right_on="zoning_id", how="left"
    )
    ...
    for btype in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES:
        null_btype_count = len(
            basis_boc.loc[basis_boc["{}_basis".format(btype)].isnull()]
        )
        logger.info(
            "Number of parcels missing allowable type for {}: {:,} or {:.1f}%".format(
                btype, null_btype_count, 100.0 * null_btype_count / len(basis_boc)
            )
        )

    # merge basis plu to p10 + pba40 plu
    p10_basis_pba40_boc = pd.merge(
        left=p10_pba40_plu,
        right=basis_boc,
        left_on="PARCEL_ID",
        right_on="parcel_id_basis",
        how="left",
    )

    p10_basis_pba40_boc.drop(
    ...
    zmod = pd.read_csv(zmod_file, usecols=["PARCEL_ID", "juris", "fbpzoningm", "nodev"])
    ...
    p10_basis_pba40_boc_zmod = pd.merge(
        left=p10_basis_pba40_boc,
        right=zmod,
        left_on="PARCEL_ID",
        right_on="PARCEL_ID_zmod",
        how="left",
    )
    ...
    juris_county_lookup_file = os.path.join(
        GITHUB_PETRALE_DIR, "zones\\jurisdictions\\juris_county_id.csv"
    )
    ...
    p10_basis_pba40_boc_zmod_withJuris = p10_basis_pba40_boc_zmod.merge(
        juris_county_lookup,
        left_on="juris_zmod",
    ...
    allowed_basis = dev_capacity_calculation_module.set_allow_dev_type(
        p10_basis_pba40_boc_zmod_withJuris, "basis"
    )
    allowed_pba40 = dev_capacity_calculation_module.set_allow_dev_type(
        p10_basis_pba40_boc_zmod_withJuris, "pba40"
    )

    p10_basis_pba40_boc_zmod_withJuris = pd.merge(
        left=p10_basis_pba40_boc_zmod_withJuris,
        right=allowed_basis,
        how="left",
        on="PARCEL_ID",
    )
    ...
    p10_basis_pba40_boc_zmod_withJuris = impute_basis_devtypes_from_pba40(
        p10_basis_pba40_boc_zmod_withJuris
    )
    # and impute basis max height from pba40 - do this before max_dua/max_far since it's used for that
    p10_basis_pba40_boc_zmod_withJuris = impute_basis_max_height_from_pba40(
        p10_basis_pba40_boc_zmod_withJuris
    )
    ...
    logger.info(
        "Parcels count after all imputations: {:,}".format(
            len(p10_basis_pba40_boc_zmod_withJuris)
        )
    )
    p10_basis_pba40_boc_zmod_withJuris.drop(
        columns=["max_dua_basis", "max_dua_pba40", "max_far_basis", "max_far_pba40"],
        inplace=True,
    )
    ...
    output_columns = [
        ...
        "plu_id_basis",
        "plu_code_basis",
        "plu_jurisdiction_basis",
        "plu_description_basis",
    ]
    ...
    for devType in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES:
        for boc_source in ["basis", "pba40"]:
            ...
            logger.info(
                "Missing {} {} parcel count: {}".format(boc_source, devType, miss_idx)
            )

    logger.info(
        "Export pba40/BASIS combined base zoning data: {} records of the following fields: {}".format(
            len(plu_boc_output), plu_boc_output.dtypes
        )
    )

    plu_boc_output.to_csv(
        os.path.join(DATA_OUTPUT_DIR, today + "_p10_plu_boc_allAttrs.csv"), index=False
    )
    ...
    for devType in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES:
        ...
        plu_boc.loc[
            (plu_boc[devType + "_pba40"] == 1) & (plu_boc[devType + "_basis"] == 0),
            devType + "_comp",
        ] = "only PBA40 allow"
        plu_boc.loc[
            (plu_boc[devType + "_pba40"] == 0) & (plu_boc[devType + "_basis"] == 1),
            devType + "_comp",
        ] = "only BASIS allow"
        plu_boc.loc[
            (plu_boc[devType + "_pba40"] == 1) & (plu_boc[devType + "_basis"] == 1),
            devType + "_comp",
        ] = "both allow"
        plu_boc.loc[
            (plu_boc[devType + "_pba40"] == 0) & (plu_boc[devType + "_basis"] == 0),
            devType + "_comp",
        ] = "both not allow"
        plu_boc.loc[
            (plu_boc[devType + "_basis"].isnull())
            & (plu_boc[devType + "_pba40"].notnull()),
            devType + "_comp",
        ] = "missing BASIS data"
        plu_boc.loc[
            plu_boc[devType + "_pba40"].isnull(), devType + "_comp"
        ] = "missing PBA40 data"
        plu_boc.loc[plu_boc["nodev_zmod"] == 1, devType + "_comp"] = "not developable"

    devType_comp = plu_boc[
        [
            "PARCEL_ID",
            "county_id",
            "county_name",
    ...
    devType_comp.to_csv(
        os.path.join(QA_QC_DIR, today + "_devType_comparison.csv"), index=False
    )
    ...
    missing_plu_id_basis = plu_boc_output.loc[
        (plu_boc_output["plu_id_basis"].isnull())
        & (plu_boc_output["zoning_id_pba40"].notnull())
    ][
        [
            "PARCEL_ID",
            "juris_zmod",
    ...
    missing_plu_id_basis.to_csv(
        os.path.join(QA_QC_DIR, today + "_missing_plu_id_basis.csv"), index=False
    )

    logger.info("Export parcels that have a plu_id_basis but no zoning_id_pba40")
    missing_zoning_id_pba40 = plu_boc_output.loc[
        (plu_boc_output["zoning_id_pba40"].isnull())
        & (plu_boc_output["plu_id_basis"].notnull())
    ][
        [
            "PARCEL_ID",
            "juris_zmod",
    ...
    missing_zoning_id_pba40.to_csv(
        os.path.join(QA_QC_DIR, today + "_missing_zoning_id_pba40.csv"), index=False
    )
