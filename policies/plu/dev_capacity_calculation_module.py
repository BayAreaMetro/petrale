...
JURIS_COUNTY_FILE = os.path.join(
...
def get_jurisdiction_county_df():
    ...
    juris_df = pd.read_csv(
        JURIS_COUNTY_FILE, usecols=["juris_name_full", "county_name"]
    )
...
def set_allow_dev_type(df_original, boc_source):
    ...
    for dev_type in ALLOWED_BUILDING_TYPE_CODES:
        df[dev_type + "_" + boc_source] = df[dev_type + "_" + boc_source].fillna(
            value=0.0
        )

    # allow_res is sum of allowed building types that are residential
    res_allowed_columns = [
        btype + "_" + boc_source for btype in RES_BUILDING_TYPE_CODES
    ]
    ...
    nonres_allowed_columns = [
        btype + "_" + boc_source for btype in NONRES_BUILDING_TYPE_CODES
    ]
    df["allow_nonres_" + boc_source] = df[nonres_allowed_columns].sum(axis=1)

    return df[["PARCEL_ID", "allow_res_" + boc_source, "allow_nonres_" + boc_source]]
...
def create_hybrid_parcel_data_from_juris_idx(logger, df_original, hybrid_idx):
    ...
    * XX_urbansim: the value of XX_basis if XX_idx==USE_BASIS otherwise the value of XX_pba40
    ...
    for dev_type in ALLOWED_BUILDING_TYPE_CODES:
        # default to BASIS
        urbansim_df["{}_urbansim".format(dev_type)] = urbansim_df[
            "{}_basis".format(dev_type)
        ]
        # but set to PBA40 if the idx says to use PBA40
        urbansim_df.loc[
            urbansim_df["{}_idx".format(dev_type)] == USE_PBA40,
            "{}_urbansim".format(dev_type),
        ] = urbansim_df["{}_pba40".format(dev_type)]
    ...
    for intensity in INTENSITY_CODES:
        # default to BASIS
        urbansim_df["max_{}_urbansim".format(intensity)] = urbansim_df[
            "max_{}_basis".format(intensity)
        ]
        # but set to PBA40 if the idx says to use PBA40
        urbansim_df.loc[
            urbansim_df["max_{}_idx".format(intensity)] == USE_PBA40,
            "max_{}_urbansim".format(intensity),
        ] = urbansim_df["max_{}_pba40".format(intensity)]
...
def calculate_capacity(df_original, boc_source, nodev_source, pass_thru_cols=[]):
    ...
    zero_unit_idx = (df["allow_res_" + boc_source] == 0) | (
        df["nodev_" + nodev_source] == 1
    )
    df.loc[zero_unit_idx, "zoned_du_" + boc_source] = 0

    # FAR calculations apply to parcels 'allowNonRes'
    df["zoned_sqft_" + boc_source] = (
        df["ACRES"] * df["max_far_" + boc_source] * SQUARE_FEET_PER_ACRE
    )

    # zero out sqft for 'nodev' parcels or parcels that don't allow non-residential
    zero_sqft_idx = (df["allow_nonres_" + boc_source] == 0) | (
        df["nodev_" + nodev_source] == 1
    )
    ...
    for dev_type in NONRES_BUILDING_TYPE_CODES:
        df.loc[df[dev_type + "_" + boc_source] == 1, "job_spaces_" + boc_source] = (
            df["zoned_sqft_" + boc_source] / SQUARE_FEET_PER_EMPLOYEE[dev_type]
        )
...
def calculate_net_capacity(
    ...
    """
    capacity_raw = calculate_capacity(
        df_original, boc_source, nodev_source, pass_thru_cols=net_pass_thru_cols
    )
    ...
    building_parcel_df.loc[
        building_parcel_df.development_type_id == 0, "building_vacant"
    ] = 1.0
    building_parcel_df.loc[
        building_parcel_df.development_type_id == 15, "building_vacant"
    ] = 1.0
    ...
    building_groupby_parcel = building_parcel_df.groupby(["PARCEL_ID"]).agg(
        {
            ...
            "year_built": "min",
            "building_id": "min",
            "building_vacant": "prod",
        }
    )  # all buildings must be vacant to call this building_vacant

    # Identify vacant parcels
    building_groupby_parcel["parcel_vacant"] = False
    building_groupby_parcel.loc[
        building_groupby_parcel["building_id"].isnull(), "parcel_vacant"
    ] = True
    building_groupby_parcel.loc[
        building_groupby_parcel["building_vacant"] == 1.0, "parcel_vacant"
    ] = True
    building_groupby_parcel.loc[
        (building_groupby_parcel["improvement_value"] == 0)
        ...
        & (building_groupby_parcel["building_sqft"] == 0),
        "parcel_vacant",
    ] = True
    logger.info(
        "Vacant parcel statistics: \n {}".format(
            building_groupby_parcel.parcel_vacant.value_counts()
        )
    )
    ...
    building_groupby_parcel.loc[
        building_groupby_parcel.year_built >= 2000, "building_age"
    ] = "after 2000"
    building_groupby_parcel.loc[
        building_groupby_parcel.year_built < 2000, "building_age"
    ] = "1980-2000"
    building_groupby_parcel.loc[
        building_groupby_parcel.year_built < 1980, "building_age"
    ] = "1940-1980"
    building_groupby_parcel.loc[
        building_groupby_parcel.year_built < 1940, "building_age"
    ] = "before 1940"

    building_groupby_parcel["has_old_building"] = False
    building_groupby_parcel.loc[
        building_groupby_parcel.building_age == "before 1940", "has_old_building"
    ] = True
    logger.info(
        "Parcel statistics by the age of the oldest building: \n {}".format(
            building_groupby_parcel.building_age.value_counts()
        )
    )

    # Identify single-family parcels smaller than 0.5 acre
    building_groupby_parcel["small_HS_parcel"] = False
    small_HS_idx = (building_groupby_parcel.residential_units == 1.0) & (
        building_groupby_parcel.ACRES < 0.5
    )
    building_groupby_parcel.loc[small_HS_idx, "small_HS_parcel"] = True
    logger.info(
        "Small single-family parcel statistics: \n {}".format(
            building_groupby_parcel.small_HS_parcel.value_counts()
        )
    )
    ...
    logger.info(
        "Small parcel (<2000 sqft) statistics: \n {}".format(
            building_groupby_parcel.small_parcel.value_counts()
        )
    )

    # Calculate parcel's investment-land ratio
    building_groupby_parcel["ILR"] = (
        building_groupby_parcel["improvement_value"]
        / building_groupby_parcel["LAND_VALUE"]
    )
    building_groupby_parcel.loc[
        building_groupby_parcel["LAND_VALUE"] == 0, "ILR"
    ] = "n/a"

    # join to raw capacity dataframe
    capacity_with_building = pd.merge(
        left=capacity_raw, right=building_groupby_parcel, how="left", on="PARCEL_ID"
    )

    # Identify under-built parcels and calculate the net units capacity for under-built parcels
    new_units = (
        capacity_with_building["zoned_du_" + boc_source]
        - capacity_with_building["residential_units"]
        - capacity_with_building["non_residential_sqft"] / SQUARE_FEET_PER_DU
    ).clip(lower=0)
    ...
    capacity_with_building["nonres_zoned_existing_ratio_" + boc_source] = (
        (
            capacity_with_building["non_residential_sqft"]
            / capacity_with_building["zoned_sqft_" + boc_source]
        )
        .replace(np.inf, 1)
        .clip(lower=0)
    )
    ...
    for capacity_type in ["zoned_du", "zoned_sqft", "zoned_Ksqft", "job_spaces"]:
        capacity_with_building[
            capacity_type + "_vacant_" + boc_source
        ] = capacity_with_building[capacity_type + "_" + boc_source]
        capacity_with_building.loc[
            capacity_with_building.parcel_vacant == False,
            capacity_type + "_vacant_" + boc_source,
        ] = 0
    ...
    for capacity_type in ["zoned_du", "zoned_sqft", "zoned_Ksqft", "job_spaces"]:
        capacity_with_building[
            capacity_type + "_underbuild_" + boc_source
        ] = capacity_with_building[capacity_type + "_" + boc_source]
        capacity_with_building.loc[
    ...
    for capacity_type in ["zoned_du", "zoned_sqft", "zoned_Ksqft", "job_spaces"]:
        capacity_with_building[
            capacity_type + "_underbuild_noProt_" + boc_source
        ] = capacity_with_building[capacity_type + "_" + boc_source]
        capacity_with_building.loc[
            (capacity_with_building["is_under_built_" + boc_source] == False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "hybrid_zoning", metavar="hybrid_zoning.csv", help="Input hybrid zoning"
    )
    parser.add_argument(
        "hybrid_version",
        metavar="hybrid_version",
        help="Version of input hybrid zoning",
    )
    parser.add_argument(
        "capacity_output", metavar="capacity_output", help="Capacity output folder"
    )
    # parser.add_argument('upzoning_zmods',   metavar='zoningmods.csv',     help='Zoningmods for upzoning')
    parser.add_argument(
        "upzoning_scenario",
        metavar="upzoning_scenario",
        help="Scenario of input upzoning zoning",
    )
