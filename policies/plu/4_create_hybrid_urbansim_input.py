...
PREV_ZONING_PARCELS_FILE = os.path.join(
    M_DIR, "Horizon", "Large General Input Data", "2015_12_21_zoning_parcels.csv"
)
...
PARCEL_UGB_TYPE_FILE = os.path.join(
    M_DIR,
    "Final_Blueprint",
    "Large General Input Data",
    "2020_11_10_parcels_geography.csv",
)
...
HYBRID_ZONING_OUTPUT_DIR = os.path.join(
    BOX_DIR, "Policies\\Base zoning\\outputs\\hybrid_base_zoning"
)
...
if __name__ == "__main__":
    ...
    if args.test == False:
        LOG_FILE = os.path.join(
            HYBRID_ZONING_OUTPUT_DIR, "{}_{}".format(today, LOG_FILE)
        )
        HYBRID_PARCELS_FILE = os.path.join(
            HYBRID_ZONING_OUTPUT_DIR, "{}_{}".format(today, HYBRID_PARCELS_FILE)
        )
        ZONING_PARCELS_FILE = os.path.join(
            HYBRID_ZONING_OUTPUT_DIR, "{}_{}".format(today, ZONING_PARCELS_FILE)
        )
        ZONING_LOOKUP_FILE = os.path.join(
            HYBRID_ZONING_OUTPUT_DIR, "{}_{}".format(today, ZONING_LOOKUP_FILE)
        )
    ...
    plu_boc_hybrid = (
        dev_capacity_calculation_module.create_hybrid_parcel_data_from_juris_idx(
            logger, plu_boc, hybrid_idx
        )
    )
    plu_boc_hybrid = pd.merge(
        left=plu_boc, right=plu_boc_hybrid, how="left", on=["PARCEL_ID", "juris_zmod"]
    )

    # calculate 'allow_res' and 'allow_nonres' based on the allowable development type
    allowed_urbansim = dev_capacity_calculation_module.set_allow_dev_type(
        plu_boc_hybrid, "urbansim"
    )

    plu_boc_hybrid = pd.merge(
        left=plu_boc_hybrid, right=allowed_urbansim, on="PARCEL_ID", how="left"
    )

    # force max_dua_urbansim to 0 to avoid over-estimating max_dua during imputation
    # two options:
    ...
    parcel_UBG_type = pd.read_csv(
        PARCEL_UGB_TYPE_FILE, usecols=["PARCEL_ID", "fbp_exp2020_id"]
    )
    logger.info(
        "Read {} lines from {}".format(len(parcel_UBG_type), PARCEL_UGB_TYPE_FILE)
    )

    parcel_UBG_type["UGB_type"] = "outside UGB"
    parcel_UBG_type.loc[
        (parcel_UBG_type.fbp_exp2020_id == "in")
        | (parcel_UBG_type.fbp_exp2020_id == "inun"),
        "UGB_type",
    ] = "inside UGB"
    logger.info(parcel_UBG_type.UGB_type.value_counts())

    plu_boc_hybrid = pd.merge(
        left=plu_boc_hybrid, right=parcel_UBG_type, on="PARCEL_ID", how="left"
    )
    # option 1
    # plu_boc_hybrid.loc[(plu_boc_hybrid.UGB_type == 'outside UGB') & (plu_boc_hybrid.allow_res_urbansim == 0),
    #                    'max_dua_urbansim'] = 0

    # option 2
    plu_boc_hybrid.loc[plu_boc_hybrid.allow_res_urbansim == 0, "max_dua_urbansim"] = 0

    logger.info(
        "Saving hybrid zoning for {} parcels to {}".format(
            len(plu_boc_hybrid), HYBRID_PARCELS_FILE
        )
    )
    ...
    zoning_lookup_pba50 = plu_boc_urbansim[
    ...
    logger.info(
        "Zoning lookup table has {} unique types (juris + zoning attributes), header:".format(
            len(zoning_lookup_pba50)
        )
    )
    logger.info(zoning_lookup_pba50.head())

    # create zoning_parcels file and attach zoning_id
    plu_boc_urbansim_ID = plu_boc_urbansim.merge(
        zoning_lookup_pba50, on=list(zoning_lookup_pba50)[:-1], how="left"
    )
    zoning_parcels_pba50 = plu_boc_urbansim_ID[
        [
            "PARCEL_ID",
            "geom_id",
            "juris_zmod",
    ...
    zoning_parcels_prev = pd.read_csv(
        PREV_ZONING_PARCELS_FILE, usecols=["geom_id", "prop"]
    )
    zoning_parcels_pba50 = zoning_parcels_pba50.merge(
        zoning_parcels_prev, on="geom_id", how="left"
    )
    ...
    zoning_names["name_pba40"] = zoning_names["name_pba40"].apply(
        lambda x: str(x) + "_pba40"
    )
    zoning_names["plu_code_basis"] = zoning_names["plu_code_basis"].apply(
        lambda x: str(x) + "_basis"
    )
    parcel_zoning_names = zoning_parcels_pba50[["PARCEL_ID", "zoning_id_pba50"]].merge(
        zoning_names, on="PARCEL_ID", how="left"
    )
    ...
    ] = parcel_zoning_names.loc[name_null_index, "plu_code_basis"]

    # find the most frenquent zoning name of each zoning_id
    name_by_zone = (
        parcel_zoning_names[["zoning_id_pba50", "zoning_name_pba50"]]
        .groupby(["zoning_id_pba50"])
        .agg(lambda x: x.value_counts().index[0])
        .reset_index()
    )
    zoning_lookup_pba50 = zoning_lookup_pba50.merge(
        name_by_zone, on="zoning_id_pba50", how="left"
    )
    # attach zoning name to the zoning lookup table
    zoning_lookup_pba50 = zoning_lookup_pba50[
        [
            "zoning_id_pba50",
            "juris_zmod",
            "zoning_name_pba50",
    ...
    zoning_lookup_pba50.rename(
        columns={
            "zoning_id_pba50": "id",
            "juris_zmod": "juris",
            "zoning_name_pba50": "name",
        },
        inplace=True,
    )
    logger.info(
        "zoning_lookup has {} unique zoning_ids; zoning_lookup table header:".format(
            len(zoning_lookup_pba50)
        )
    )
    logger.info(zoning_lookup_pba50.head())
    ...
    logger.info(
        "zoning_parcels_pba50 is length {} but has {} rows with null geom_id; removing".format(
            len(zoning_parcels_pba50),
            len(zoning_parcels_pba50.loc[pd.isnull(zoning_parcels_pba50["geom_id"])]),
        )
    )
    zoning_parcels_pba50 = zoning_parcels_pba50.loc[
        pd.notnull(zoning_parcels_pba50["geom_id"]),
    ]
    logger.info("zoning_parcels_pba50 is length {}".format(len(zoning_parcels_pba50)))

    # so we can convert geom_id to int
    zoning_parcels_pba50["geom_id"] = (
        zoning_parcels_pba50["geom_id"].round(0).astype(np.int64)
    )

    logger.info(
        "zoning_parcels_pba50 has {} records; table head:\n{}".format(
            len(zoning_parcels_pba50), zoning_parcels_pba50.head()
        )
    )

    # export
    logger.info(
        "Export zoning_lookup table with the following attributes: {}".format(
            zoning_lookup_pba50.dtypes
        )
    )
    zoning_parcels_pba50.to_csv(ZONING_PARCELS_FILE, index=False)

    logger.info(
        "Export zoning_parcels table with the following attributes: {}".format(
            zoning_parcels_pba50.dtypes
        )
    )
