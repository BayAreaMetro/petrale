...
if os.getenv("USERNAME") == "ywang":
    ...
    BOX_SMELT_DIR = "C:\\Users\\{}\\Box\\baydata\\smelt\\2020 03 12".format(
        os.getenv("USERNAME")
    )
...
HORIZON_ZONING_BOX_DIR = os.path.join(
    M_WORKING_DIR, "Horizon", "Large General Input Data"
)
PBA50_ZONINGMOD_DIR = os.path.join(
    M_WORKING_DIR, "Final_Blueprint", "Zoning Modifications"
)
...
# outputs locations
PBA50_LARGE_INPUT_DIR = os.path.join(
    BOX_DIR, "PBA50", "Current PBA50 Large General Input Data"
)
M_LARGE_INPUT_FB_DIR = os.path.join(
    M_WORKING_DIR, "Final_Blueprint", "Large General Input Data"
)
M_LARGE_INPUT_EIR_DIR = os.path.join(M_WORKING_DIR, "EIR", "Large General Input Data")
LOG_FILE = os.path.join(
    M_LARGE_INPUT_EIR_DIR, "{}_update_parcels_geography.log".format(today)
)
...
if __name__ == "__main__":
    ...
    logger.info(
        "Read {} records from {}, with {} unique Parcel IDs, and header: \n {}".format(
            len(basemap_p10),
            basemap_p10_file,
    ...
    pg_pba40_file = os.path.join(
        HORIZON_ZONING_BOX_DIR, "07_11_2019_parcels_geography.csv"
    )
    pg_pba40_cols = [
        "geom_id",
        "pda_id",
    ...
    pg_pba40 = pd.read_csv(pg_pba40_file, usecols=pg_pba40_cols)
    pg_pba40.rename(columns={"pda_id": "pda_id_pba40"}, inplace=True)

    logger.info(
        "Read {} records from {}, with header: \n {}".format(
            len(pg_pba40), pg_pba40_file, pg_pba40.head()
        )
    )
    ...
    pba50_attrs_file = os.path.join(
        EIR_ZONINGMOD_DIR, "p10_pba50_EIR_attr_20210224.csv"
    )
    pba50_attrs_cols = [
        "geom_id_s",
        "juris_id",
    ...
    pba50_attrs = pd.read_csv(pba50_attrs_file, usecols=pba50_attrs_cols)
    pba50_attrs.geom_id_s = pba50_attrs.geom_id_s.apply(lambda x: int(round(x)))
    pba50_attrs.rename(
        columns={
            "pba50zonin": "pba50zoningmodcat",
    ...
    logger.info(
        "Read {} records from {}, with header: \n {}".format(
            len(pba50_attrs), pba50_attrs_file, pba50_attrs.head()
        )
    )
    ...
    logger.info(
        "Read {} records from {}, with {} unique Parcel IDs, header: \n {}".format(
            len(pda_pba50_db),
            pda_pba50_db_file,
    ...
    juris_code = pd.read_csv(
        juris_code_file, usecols=["jurisdiction_id", "juris_id", "juris_name_full"]
    )

    ## Join pab50 attributes to pba40 parcel set
    pg_pba50_merge = (
        pg_pba40.merge(pba50_attrs, left_on="geom_id", right_on="geom_id_s", how="left")
        .merge(juris_code, on="juris_id", how="left")
        .merge(basemap_p10, on="geom_id_s", how="left")
        .merge(pda_pba50_db, left_on="PARCEL_ID", right_on="parcel_id", how="left")
        .merge(pda_pba50_fb, left_on="PARCEL_ID", right_on="parcel_id", how="left")
    )
    ...
    for filename in list(glob.glob(nodev_folder)):
        if "noDev_parcels_" in filename:
            ...
            if "PARCEL_ID" in nodev_parcel:
                logger.info(
                    "update {} parcels using PARCEL_ID".format(len(nodev_parcel))
                )
                nodev_parcel.PARCEL_ID = nodev_parcel.PARCEL_ID.apply(
                    lambda x: int(round(x))
                )
                pg_pba50_merge = pg_pba50_merge.merge(
                    nodev_parcel[["PARCEL_ID", "nodev_update"]],
                    on="PARCEL_ID",
                    how="left",
                )
                logger.info(
                    pg_pba50_merge.loc[pg_pba50_merge.nodev_update.notnull()][
                        ["PARCEL_ID", "nodev", "nodev_update"]
                    ]
                )
                pg_pba50_merge.loc[
                    pg_pba50_merge.nodev_update.notnull(), "nodev"
                ] = pg_pba50_merge.loc[
                    pg_pba50_merge.nodev_update.notnull(), "nodev_update"
                ]
                ...
                nodev_parcel.geom_id = nodev_parcel.geom_id.apply(
                    lambda x: int(round(x))
                )
                pg_pba50_merge = pg_pba50_merge.merge(
                    nodev_parcel[["geom_id", "nodev_update"]], on="geom_id", how="left"
                )
                logger.info(
                    pg_pba50_merge.loc[pg_pba50_merge.nodev_update.notnull()][
                        ["geom_id", "nodev", "nodev_update"]
                    ]
                )
                pg_pba50_merge.loc[
                    pg_pba50_merge.nodev_update.notnull(), "nodev"
    ...
    logger.info(
        "Export {} records with {} unique PARCEL IDs to {} with the following fields: \n {}".format(
            len(pg_all),
    ...
    pg_all.to_csv(
        os.path.join(PBA50_LARGE_INPUT_DIR, today + "_parcels_geography.csv"),
        index=False,
    )
