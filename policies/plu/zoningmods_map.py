...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-parcels_geography",
        help="Parcels geography layer (maps parcels to zoning mod category)",
    )
    parser.add_argument(
        "-zmods_csv",
        metavar="zmods.csv",
        help="Zoning mods definition for zoning mod categories",
    )
    parser.add_argument(
        "-zmodcat_col",
        help="Zoning mod category column. e.g. pba50zoningmodcat or fbpzoningmodcat",
    )
    parser.add_argument(
        "-join_field", metavar="join_field", help="Join field for parcel-zmods join"
    )
    parser.add_argument(
        "-join_type",
        choices=["KEEP_ALL", "KEEP_COMMON"],
        default="KEEP_ALL",
    ...
    zmod_attr = pandas.read_csv(
        os.path.join(args.folder, args.parcels_geography), usecols=zmod_cols
    )
    print(
        "Read {} records from {}\nwith {} unique {} and {} unique {}".format(
            len(zmod_attr),
            os.path.join(args.folder, args.parcels_geography),
            len(zmod_attr[args.join_field].unique()),
            args.join_field,
            len(zmod_attr[args.zmodcat_col].unique()),
            args.zmodcat_col,
        )
    )
    ...
    print(
        "Copy {} to {}".format(
            args.parcels_geography, os.path.join(args.folder, args.output_gdb)
        )
    )
    ...
    print(
        "Created {} with {} records".format(
            zmod_attr_table_path, arcpy.GetCount_management(zmod_attr_table_path)
        )
    )
    ...
    print("Copy {} to {}".format(p10, os.path.join(args.folder, args.output_gdb)))
    ...
    arcpy.CopyFeatures_management(
        os.path.join(args.folder, args.input_gdb, args.p10_layer),
    ...
    p_zmod_attr_join = arcpy.AddJoin_management(
        args.p10_layer,
        args.join_field,
        zmod_attr_table,
        args.join_field,
    ...
    # save it
    arcpy.CopyFeatures_management(
        p_zmod_attr_join, os.path.join(args.folder, args.output_gdb, p_zmod_attr_joined)
    )
    print(
        "Completed creation of {}".format(
            os.path.join(args.folder, args.output_gdb, p_zmod_attr_joined)
        )
    )
    field_names = [f.name for f in arcpy.ListFields(p_zmod_attr_joined)]
    print("{} has the following fields: {}".format(p_zmod_attr_joined, field_names))

    ########## Dissolve the joint parcel-zmods layer by zoningmod category ##########

    print(
        "Dissolve {} on field: {}".format(
            p_zmod_attr_joined, [zmod_attr_table + "_" + args.zmodcat_col]
        )
    )
    ...
    print(
        "Dissolve completed; {} has {} records and the following fields \n{}".format(
            p_zmod_dissolved, arcpy.GetCount_management(p_zmod_dissolved), field_names
        )
    )
    ...
    print(
        "Read {} records from {}, with {} unique {} and the following fields: \n{}".format(
            len(zmods),
            args.zmods_csv,
            len(zmods[args.zmodcat_col].unique()),
            args.zmodcat_col,
            list(zmods),
        )
    )
    ...
    print(
        "Copy {} to {}".format(
            args.zmods_csv, os.path.join(args.folder, args.output_gdb)
        )
    )
    ...
    arcpy.da.NumPyArrayToTable(
        zmods_values, os.path.join(args.folder, args.output_gdb, zmods_table)
    )
    ...
    p_zmods_join = arcpy.AddJoin_management(
        p_zmod_dissolved,
        zmod_attr_table + "_" + args.zmodcat_col,
        zmods_table,
        args.zmodcat_col,
    ...
    # save it
    arcpy.CopyFeatures_management(
        p_zmods_join, os.path.join(args.folder, args.output_gdb, p_zmods_joined)
    )
    print(
        "Completed creation of {}".format(
            os.path.join(args.folder, args.output_gdb, p_zmods_joined)
        )
    )

    print(
        "{} has {} records".format(
            p_zmods_joined, arcpy.GetCount_management(p_zmods_joined)[0]
        )
    )
