...
COUNTY_JURISDICTIONS_CSV = (
    "M:\\Data\\GIS layers\\Jurisdictions\\county_jurisdictions.csv"
)
...
if os.getenv("USERNAME") == "lzorn":
    ...
    ARCGIS_PROJECTS = [
        os.path.join(WORKSPACE_DIR, "UrbanSim_BASIS_zoning_intensity_fb.aprx"),
    ...
    HYBRID_CONFIG_DIR = os.path.join(
        PETRALE_GITHUB_DIR, "policies", "plu", "base_zoning", "hybrid_index"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug",
        help="If on, saves a copy of the arcgis project with mods.",
        action="store_true",
    )
    parser.add_argument("--restart_juris", help="Jurisdiction to restart from")
    parser.add_argument(
        "--jurisdiction",
        help="Jurisdiction. If none passed, will process all",
        nargs="+",
    )
    parser.add_argument(
        "--metric",
        help="Metrics type(s). If none passed, will process all",
        nargs="+",
        choices=[
            "DUA",
            "FAR",
    ...
    parser.add_argument(
        "--hybrid_config",
        help="Required arg. Hybrid config file in {}".format(HYBRID_CONFIG_DIR),
        required=True,
    )
    parser.add_argument(
        "--output_type",
        help="Type of map to export",
        choices=["pdf", "png"],
        default="pdf",
    )
    ...
    hybrid_config_df = pandas.read_csv(
        os.path.join(HYBRID_CONFIG_DIR, args.hybrid_config)
    )
    ...
    if args.jurisdiction:
        ...
        for juris in args.jurisdiction:
            if juris not in JURISDICTION_TO_COUNTY:
                print(
                    "Jurisdiction [{}] not found in {}".format(
                        juris, COUNTY_JURISDICTIONS_CSV
                    )
                )
            else:
    ...
    METRICS_DEF = collections.OrderedDict(
        [
            ...
            (
                "DUA",
                [
                    "UrbanSim_BASIS_zoning_intensity.aprx",
                    "DUA",
                    "Check Residential Densities",
                    "max_dua_idx",
                ],
            ),
            (
            ...
            (
                ...
                [
                    "UrbanSim_BASIS_zoning_intensity.aprx",
                    "height",
                    "Check Allowable Building Heights",
                    "max_height_idx",
                ],
            ),
            # residential
            (
                "HS",
                [
                    "UrbanSim_BASIS_zoning_devType.aprx",
                    "Allow HS (Single-family Housing)",
                    None,
                    "HS_idx",
                ],
            ),
            ...
            (
                ...
                [
                    "UrbanSim_BASIS_zoning_devType.aprx",
                    "Allow MR (Mixed-use Residential)",
                    None,
                    "MR_idx",
                ],
            ),
            ...
            try:
                ...
                if basis_check_col:
                    if jurisdiction not in juris_review_dict:
                        print(
                            "Couldn't find jurisdiction {} in BASIS jurisdiction review {}".format(
                                jurisdiction, JURIS_REVIEW
                            )
                        )
                    else:
                        basis_check_val = juris_review_dict[jurisdiction][
                            basis_check_col
                        ]
                        print(
                            "  BASIS check val for {}: {}".format(
                                basis_check_col, basis_check_val
                            )
                        )
                basis_hybrid_val = hybrid_config_dict[juris_code][basis_hybrid_col]
                print(
                    "  BASIS hybrid config val for {}: {}".format(
                        basis_hybrid_col, basis_hybrid_val
                    )
                )
                ...
                if (
                    metric
                    in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES
                ):
                    is_devtype = True
                    map_metric = prev_allowdevtype_metric
                    map_metric_name = METRICS_DEF[prev_allowdevtype_metric][1]
                print(
                    "   map_metric:[{}]  map_metric_name:[{}]".format(
                        map_metric, map_metric_name
                    )
                )
                ...
                for my_map in maps:
                    if my_map.name.endswith(map_metric) or my_map.name.endswith(
                        map_metric_name
                    ):
                        # process this one
                    ...
                    for layer in my_map.listLayers():
                        ...
                        print(
                            "      Definition query: {}".format(layer.definitionQuery)
                        )
                        # modify to current jurisdiction
                        layer.definitionQuery = layer.definitionQuery.replace(
                            prev_jurisdiction, jurisdiction
                        )
                        layer.definitionQuery = layer.definitionQuery.replace(
                            prev_juris_code, juris_code
                        )
                        # modify to current devtype
                        if is_devtype:
                            layer.definitionQuery = layer.definitionQuery.replace(
                                prev_allowdevtype_metric, metric
                            )
                        ...
                        if is_devtype:
                            ...
                            if (
                                hasattr(layer.symbology, "renderer")
                                and layer.symbology.renderer.type
                                == "UniqueValueRenderer"
                            ):

                                fields = layer.symbology.renderer.fields
                                new_fields = [
                                    field.replace(prev_allowdevtype_metric, metric)
                                    for field in fields
                                ]
                                ...
                                print(
                                    "      Symbology.renderer.fields: {} => {}".format(
                                        fields, new_fields
                                    )
                                )
                                for grp in sym.renderer.groups:
                                    for itm in grp.items:
                                        if itm.values == [["0"]]:
                                            ...
                                            print(
                                                "      Don't recognize itm.values: {}".format(
                                                    itm.values
                                                )
                                            )
                                layer.symbology = sym
                ...
                for element in layout.listElements():
                    ...
                    if element.name == "juris_review_false":
                        element.visible = (
                            not basis_check_val
                        )  # visible if basis_check_val==False
                    if element.name == "juris_review_true":
                        element.visible = (
                            basis_check_val  # visible if basis_check_val==True
                        )

                    if element.name == "arrow_basis":
                        element.visible = (
                            basis_hybrid_val  # visible if basis_hybrid_val==True
                        )
                    ...
                    if element.name == "arrow_pba40":
                        element.visible = (
                            not basis_hybrid_val
                        )  # visible if basis_hybrid_val==False
                ...
                if args.debug:
                    copy_filename = arc_project.replace(
                        ".aprx", "_{}_{}.aprx".format(juris_code, metric)
                    )
