...
NET_CAPACITY_CODES = [
    ...
    "job_spaces_vacant",
    "zoned_du_underbuild",
    "zoned_Ksqft_underbuild",
    "job_spaces_underbuild",
    "zoned_du_underbuild_noProt",
    "zoned_Ksqft_underbuild_noProt",
...
def apply_upzoning_to_parcel_data(
    logger, parcel_basezoning_original, upzoning_scenario, upzoning_version
):
    """
    ...
    zmods_upzoning_file = os.path.join(
        PBA50_ZONINGMODS_DIR,
    ...
    parcel_basezoning_zoningmods = parcel_basezoning.merge(
        zmods_upzoning_df, on="fbpzoningm", how="left"
    )
    ...
    for dev_type in dev_capacity_calculation_module.ALLOWED_BUILDING_TYPE_CODES:
        parcel_basezoning_zoningmods[
            "{}_{}".format(dev_type, upzoning_scenario)
    ...
    for intensity in dev_capacity_calculation_module.INTENSITY_CODES:
        parcel_basezoning_zoningmods[
            "max_{}_{}".format(intensity, upzoning_scenario)
    ...
    if len(add_bldg_types) > 0:
        for devType in add_bldg_types:
            ...
            parcel_basezoning_zoningmods.loc[
                add_bldg_parcels, devType + "_" + upzoning_scenario
            ] = 1
    ...
    if len(drop_bldg_types) > 0:
        for devType in drop_bldg_types:
            ...
            parcel_basezoning_zoningmods.loc[
                drop_bldg_parcels, devType + "_" + upzoning_scenario
            ] = 0
    ...
    for devType in add_bldg_types + drop_bldg_types:
        logger.info(
            "Out of {:,} parcels: \n {:,} parcels allow {} before applying fbpzoningm dev type adjustment;\
                    \n {:,} parcels allow {} after applying fbpzoningm dev type adjustment.".format(
                len(parcel_basezoning_zoningmods),
                len(
                    parcel_basezoning_zoningmods.loc[
                        parcel_basezoning_zoningmods[devType + "_basezoning"] == 1
                    ]
                ),
                devType,
                len(
                    parcel_basezoning_zoningmods.loc[
                        parcel_basezoning_zoningmods[devType + "_" + upzoning_scenario]
                        == 1
                    ]
                ),
                devType,
    ...
    for intensity in ["dua", "far"]:
        ...
        parcel_basezoning_zoningmods.loc[
            up_parcels, "max_{}_{}".format(intensity, upzoning_scenario)
        ] = parcel_basezoning_zoningmods[
            [
                "max_{}_{}".format(intensity, upzoning_scenario),
                "{}_up".format(intensity),
            ]
        ].max(
            axis=1
        )
        ...
        # modify intensity when 'intensity_up' is not null
        down_parcels = parcel_basezoning_zoningmods[
            "{}_down".format(intensity)
        ].notnull()

        # the effective max_dua is the larger of base zoning max_dua and the pba50 max_dua
        parcel_basezoning_zoningmods.loc[
            down_parcels, "max_{}_{}".format(intensity, upzoning_scenario)
        ] = parcel_basezoning_zoningmods[
            [
                "max_{}_{}".format(intensity, upzoning_scenario),
                "{}_down".format(intensity),
        ...
        logger.info(
            "Before applying fbpzoningm intensity adjustment: \n",
            parcel_basezoning_zoningmods[
                ["max_" + intensity + "_basezoning"]
            ].describe(),
        )
        logger.info(
            "After applying fbpzoningm intensity adjustment: \n",
            parcel_basezoning_zoningmods[
                ["max_" + intensity + "_" + upzoning_scenario]
            ].describe(),
        )

    parcel_upzoning = parcel_basezoning_zoningmods[keep_cols]
    logger.info(
        "Generate parcel-level upzoning table of {:,} records: \n {}".format(
            len(parcel_upzoning), parcel_upzoning.head()
        )
    )
...
def summary_capacity(parcel_capacity, groupby_field, capacity_metrics):
    return (
        parcel_capacity.groupby([groupby_field])[capacity_metrics].sum().reset_index()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-zoningmods_version", help="version of zoningmods, represented by the date"
    )
    parser.add_argument(
        "-attr_version", help="version of p10_pba50_attr, represented by the date"
    )
    ...
    PARCEL_ZONING_ID_FILE = os.path.join(
        URBANSIM_BASEZONING_DIR, "2020_06_22_zoning_parcels_hybrid_pba50.csv"
    )
    BASEZONING_LOOKUP_FILE = os.path.join(
        URBANSIM_BASEZONING_DIR, "2020_06_22_zoning_lookup_hybrid_pba50.csv"
    )
    # URBANSIM_PARCEL_FILE         = os.path.join(URBANSIM_BASEZONING_DIR, '2020_04_17_parcels_geography.csv')
    URBANSIM_PARCEL_TAZ_FILE = os.path.join(
        URBANSIM_BASEZONING_DIR, "2020_08_17_parcel_to_taz1454sub.csv"
    )

    PBA50_ZONINGMODS_DIR = os.path.join(M_DIR, "Zoning Modifications")
    PARCEL_ZONINGMODS_PBA50_FILE = os.path.join(
        PBA50_ZONINGMODS_DIR, "p10_pba50_attr_{}.csv".format(str(args.attr_version))
    )
    ...
    BOX_UPZONING_DIR = os.path.join(
        BOX_DIR, "Policies", "Zoning Modifications", "capacity"
    )

    COMPARE_JURIS_CAPACITY_FILE = "compare_juris_capacity_{}.csv".format(
        args.zoningmods_scenario
    )
    COMPARE_TAZ_CAPACITY_FILE = "compare_taz_capacity_{}.csv".format(
        args.zoningmods_scenario
    )
    ...
    P10_UPZONING_PBA50_FILE = "p10_upzoning_pba50_{}.csv".format(
        args.zoningmods_scenario
    )
    PARCEL_CAPACITY_BASEZONING_FILE = "parcel_capacity_basezoning.csv"
    PARCEL_CAPACITY_UPZONING_FILE = "parcel_capacity_upzoning_{}.csv".format(
        args.zoningmods_scenario
    )
    PARCEL_CAPACITY_BAUS_FILE = "parcel_capacity_baus_{}.csv".format(
        args.zoningmods_scenario
    )

    if args.test == False:
        LOG_FILE = os.path.join(BOX_UPZONING_DIR, "{}_{}".format(today, LOG_FILE))
        COMPARE_JURIS_CAPACITY_FILE = os.path.join(
            BOX_UPZONING_DIR, "{}_{}".format(today, COMPARE_JURIS_CAPACITY_FILE)
        )
        COMPARE_TAZ_CAPACITY_FILE = os.path.join(
            BOX_UPZONING_DIR, "{}_{}".format(today, COMPARE_TAZ_CAPACITY_FILE)
        )
    ...
    logger.info(
        "COMPARE_JURIS_CAPACITY_FILE     = {}".format(COMPARE_JURIS_CAPACITY_FILE)
    )
    logger.info(
        "COMPARE_TAZ_CAPACITY_FILE       = {}".format(COMPARE_TAZ_CAPACITY_FILE)
    )

    ## Read p10 parcels data
    basemap_p10_file = os.path.join(M_SMELT_DIR, "p10.csv")
    basemap_p10 = pd.read_csv(
        basemap_p10_file, usecols=["PARCEL_ID", "ACRES", "LAND_VALUE"]
    )
    ...
    parcel_zoning_id = pd.read_csv(PARCEL_ZONING_ID_FILE, usecols=parcel_use_cols)
    parcel_zoning_id.rename(columns={"PARCEL_ID": "PARCEL_ID_pz"}, inplace=True)
    basezoning_lookup = pd.read_csv(BASEZONING_LOOKUP_FILE)
    basezoning_lookup.columns = [
        col + "_basezoning" for col in basezoning_lookup.columns.values
    ]

    parcel_basezoning = parcel_zoning_id.merge(
        basezoning_lookup, left_on="zoning_id", right_on="id_basezoning", how="left"
    )
    parcel_basezoning.rename(columns={"nodev": "nodev_basezoning"}, inplace=True)
    logger.info(
        "Parcels with base zoning has {} records, with columns:\n{}".format(
            len(parcel_basezoning), parcel_basezoning.dtypes
        )
    )

    # Read PBA50 zoningmods
    parcel_zoningmods = pd.read_csv(
        PARCEL_ZONINGMODS_PBA50_FILE, usecols=["PARCEL_ID", "fbpzoningm", "juris"]
    )
    parcel_zoningmods.PARCEL_ID = parcel_zoningmods.PARCEL_ID.apply(
    ...
    p10_basezoning = basemap_p10.merge(
        parcel_basezoning, left_on="PARCEL_ID", right_on="PARCEL_ID_pz", how="left"
    ).merge(
        parcel_zoningmods, left_on="PARCEL_ID", right_on="PARCEL_ID_attr", how="left"
    )
    ...
    logger.info(
        "Running step ------ Applying upzoning {}".format(
            "zoning_mods_{}_{}.csv".format(
                args.zoningmods_scenario, args.zoningmods_version
            )
        )
    )

    p10_upzoning_pba50 = apply_upzoning_to_parcel_data(
        logger, p10_basezoning, args.zoningmods_scenario, args.zoningmods_version
    )

    logger.info(
        "Generating p10_upzoning_pba50 with {} records;\n Headers:\n{}".format(
            len(p10_upzoning_pba50), p10_upzoning_pba50.head()
        )
    )
    ...
    logger.info(
        "b10 building data has {:,} unique PARCEL_ID:".format(
            len(basemap_b10.parcel_id.unique())
        )
    )
    ...
    basezoning_allow_dev_type = dev_capacity_calculation_module.set_allow_dev_type(
        p10_upzoning_pba50, boc_source="basezoning"
    )
    upzoning_allow_dev_type = dev_capacity_calculation_module.set_allow_dev_type(
        p10_upzoning_pba50, boc_source=args.zoningmods_scenario
    )

    # put them together
    p10_upzoning_pba50 = p10_upzoning_pba50.merge(
        basezoning_allow_dev_type, on="PARCEL_ID", how="left"
    ).merge(upzoning_allow_dev_type, on="PARCEL_ID", how="left")

    # Add TAZ id to parcel data
    parcel_taz = pd.read_csv(URBANSIM_PARCEL_TAZ_FILE, usecols=["PARCEL_ID", "ZONE_ID"])
    ...
    p10_upzoning_pba50 = p10_upzoning_pba50.merge(
        parcel_taz, left_on="PARCEL_ID", right_on="PARCEL_ID_taz", how="left"
    )

    logger.debug(
        "p10_upzoning_pba50 with columns:\n{}".format(p10_upzoning_pba50.dtypes)
    )
    ...
    logger.info(
        "Running step ------ Calculating raw development capacity under basezoning"
    )

    raw_parcel_capacity_basezoning = dev_capacity_calculation_module.calculate_capacity(
        p10_upzoning_pba50,
        "basezoning",
        "basezoning",
    ...
    logger.debug(
        "raw_parcel_capacity_basezoning.head():\n{}".format(
            raw_parcel_capacity_basezoning.head()
        )
    )

    logger.info(
        "Running step ------ Calculating raw development capacity under {}".format(
            "zoning_mods_" + args.zoningmods_scenario
        )
    )
    raw_parcel_capacity_upzoning = dev_capacity_calculation_module.calculate_capacity(
        p10_upzoning_pba50,
        args.zoningmods_scenario,
        "basezoning",
        pass_thru_cols=["juris", "ZONE_ID"],
    ...
    logger.debug(
        "raw_parcel_capacity_upzoning.head():\n{}".format(
            raw_parcel_capacity_upzoning.head()
        )
    )

    logger.info(
        "Running step ------ Calculating net development capacity under basezoning"
    )

    net_parcel_capacity_basezoning = (
        dev_capacity_calculation_module.calculate_net_capacity(
            logger,
            p10_upzoning_pba50,
            "basezoning",
            "basezoning",
    ...
    logger.debug(
        "net_parcel_capacity_basezoning.head():\n{}".format(
            net_parcel_capacity_basezoning.head()
        )
    )

    logger.info(
        "Running step ------ Calculating net development capacity under {}".format(
            "zoning_mods_" + args.zoningmods_scenario
        )
    )
    net_parcel_capacity_upzoning = (
        dev_capacity_calculation_module.calculate_net_capacity(
            logger,
            p10_upzoning_pba50,
            args.zoningmods_scenario,
    ...
    # in test mode, export the data for QA/QC
    if args.test == True:
        raw_parcel_capacity_basezoning.to_csv(
            "raw_parcel_capacity_basezoning.csv", index=False
        )
        net_parcel_capacity_basezoning.to_csv(
            "net_parcel_capacity_basezoning.csv", index=False
        )
        raw_parcel_capacity_upzoning.to_csv(
            "raw_parcel_capacity_upzoning.csv", index=False
        )
        net_parcel_capacity_upzoning.to_csv(
            "net_parcel_capacity_upzoning.csv", index=False
        )

    ## calculate jurisdiction-level capacity

    juris_raw_capacity_basezoning = summary_capacity(
        raw_parcel_capacity_basezoning,
        "juris",
        [raw_metrics + "_basezoning" for raw_metrics in RAW_CAPACITY_CODES],
    )

    juris_net_capacity_basezoning = summary_capacity(
        net_parcel_capacity_basezoning,
        "juris",
        [net_metrics + "_basezoning" for net_metrics in NET_CAPACITY_CODES],
    )

    juris_raw_capacity_upzoning = summary_capacity(
        raw_parcel_capacity_upzoning,
        "juris",
        [
            raw_metrics + "_" + args.zoningmods_scenario
            for raw_metrics in RAW_CAPACITY_CODES
        ],
    )

    juris_net_capacity_upzoning = summary_capacity(
        net_parcel_capacity_upzoning,
        "juris",
        [
            net_metrics + "_" + args.zoningmods_scenario
            for net_metrics in NET_CAPACITY_CODES
        ],
    )

    ## calculate taz-level capacity
    taz_raw_capacity_basezoning = summary_capacity(
        raw_parcel_capacity_basezoning,
        "ZONE_ID",
        [raw_metrics + "_basezoning" for raw_metrics in RAW_CAPACITY_CODES],
    )

    taz_net_capacity_basezoning = summary_capacity(
        net_parcel_capacity_basezoning,
        "ZONE_ID",
        [net_metrics + "_basezoning" for net_metrics in NET_CAPACITY_CODES],
    )

    taz_raw_capacity_upzoning = summary_capacity(
        raw_parcel_capacity_upzoning,
        "ZONE_ID",
        [
            raw_metrics + "_" + args.zoningmods_scenario
    ...
    taz_net_capacity_upzoning = summary_capacity(
        net_parcel_capacity_upzoning,
        "ZONE_ID",
        [
            net_metrics + "_" + args.zoningmods_scenario
    ...
    juris_capacity_compare = (
        juris_raw_capacity_basezoning.merge(juris_net_capacity_basezoning, on="juris")
        .merge(juris_raw_capacity_upzoning, on="juris")
        .merge(juris_net_capacity_upzoning, on="juris")
    )
    juris_capacity_compare.rename(columns={"juris": "jurisdiciton"}, inplace=True)

    logger.debug(
        "juris_capacity_compare.head():\n{}".format(juris_capacity_compare.head())
    )

    # merge to generate taz-level summary
    taz_capacity_compare = (
        taz_raw_capacity_basezoning.merge(taz_net_capacity_basezoning, on="ZONE_ID")
        .merge(taz_raw_capacity_upzoning, on="ZONE_ID")
        .merge(taz_net_capacity_upzoning, on="ZONE_ID")
    )
    ...
    logger.info(
        "Export development capacity comparison by jurisdiciton: \n{}".format(
            juris_capacity_compare.dtypes
        )
    )
    ...
    logger.info(
        "Export development capacity comparison by TAZ: \n{}".format(
            taz_capacity_compare.dtypes
        )
    )
