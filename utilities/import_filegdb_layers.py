...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_layer", metavar="input_layer", help="Geometry layer in input geodatabase"
    )
    parser.add_argument(
        "input_field", metavar="input_layer", help="Join field in input_layer"
    )
    parser.add_argument("join_csv", metavar="join.csv", help="CSV layer for joining")
    parser.add_argument(
        "join_field", metavar="join_field", help="Join field in join_csv"
    )
    parser.add_argument(
        "join_type",
    ...
    print(
        "Read {} lines from {}. Head:\n{}Dtypes:\n{}".format(
            len(df), args.join_csv, df.head(), df.dtypes
        )
    )
    ...
    arcpy.CopyFeatures_management(
        os.path.join(args.input_gdb, args.input_layer),
        os.path.join(args.output_gdb, args.input_layer),
    )
    # create join layer with input_layer and join_table
    print(
        "Joining {} with {}".format(
            os.path.join(args.output_gdb, args.input_layer), table_name
        )
    )
    joined_table = arcpy.AddJoin_management(
        os.path.join(args.output_gdb, args.input_layer),
        args.input_field,
        os.path.join(args.output_gdb, table_name),
        args.join_field,
    ...
    # save it
    arcpy.CopyFeatures_management(
        joined_table, os.path.join(args.output_gdb, new_table_name)
    )
    print(
        "Completed creation of {}".format(os.path.join(args.output_gdb, new_table_name))
    )
    ...
    for field in delete_fields:
        try:
            arcpy.DeleteField_management(
                os.path.join(args.output_gdb, new_table_name), [field]
            )
