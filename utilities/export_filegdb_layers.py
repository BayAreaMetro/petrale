...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "geodatabase",
        metavar="geodatabase.gdb",
        help="File geodatabase with layer export",
    )
    ...
    field_types = {"parcel_id": "LONG", "zone_id": "SHORT", "geom_id_s": "TEXT"}
            ...
            for update_field in field_types.keys():
                if field.name.lower() == update_field:
                    ...
                    arcpy.CalculateField_management(
                        args.layer, "temp", "int(round(!field!))", "PYTHON3"
                    )
                    # delete the old field
                    arcpy.AlterField_management(
                        args.layer, f_name, f_name + "_old", f_aliasName + "_old"
                    )
    ...
    if args.layer in arcpy.ListFeatureClasses():
        ...
        print(
            "Feature Class [{}] has {} rows".format(
                os.path.join(args.geodatabase, args.layer), result[0]
            )
        )
        ...
        if args.format == "geojson":
            ...
            arcpy.FeaturesToJSON_conversion(
                os.path.join(args.geodatabase, args.layer), outfile, geoJSON="GEOJSON"
            )
        ...
        if args.format == "shp":
            ...
            arcpy.FeatureClassToShapefile_conversion(
                os.path.join(args.geodatabase, args.layer), Output_Folder="."
            )
            print("Wrote {}".format(outfile))
        ...
        if args.format == "csv":
            ...
            arcpy.CopyRows_management(
                os.path.join(args.geodatabase, args.layer), outfile
            )
            print("Wrote {}".format(outfile))
    ...
    if args.layer in arcpy.ListTables():
        ...
        if args.format == "csv":
            ...
            arcpy.TableToTable_conversion(
                os.path.join(args.geodatabase, args.layer),
                out_path=".",
                out_name=outfile,
            )
