...
list_us_runid = [
    ...
    us_2050_ALT2_EIR,
    us_2050_FBP_Final,
]  # , us_2050_FBP_Final, us_2050_DBP_Final]
...
def run_as_external_process(python_str):
    ...
    try:
        completed = subprocess.run(
            ["python", "-c", python_str], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def create_transit_features(logger, transit_type):
    ...
    if transit_type == "current":
        ...
        if transit_type == "noplan":
            ...
            new_major = arcpy.management.SelectLayerByAttribute(
                input_layer,
                "NEW_SELECTION",
        ...
        arcpy.Merge_management(
            [curprefix + "_majorbuf", prefix + "_newmajorbuf"],
            prefix + "_majorbuf_predissolve",
        )
        # dissolve
    ...
    logger.info(
        "Creating buffer for stops with headway 15-30 min => {}_hdwy30buf".format(
            prefix
        )
    )
    ...
    if transit_type == "current":
        hdwy30 = arcpy.SelectLayerByAttribute_management(
            input_layer,
            "NEW_SELECTION",
            "am_av_hdwy > 15 And am_av_hdwy <= 30 And pm_av_hdwy > 15 And pm_av_hdwy <= 30",
        )
        arcpy.CopyFeatures_management(
            hdwy30, prefix + "_hdwy30"
        )  # save selection to new feature class
    ...
    logger.info(
        "Creating buffer for stops with headway 30+ min => {}_hdwy30plusbuf".format(
            prefix
        )
    )
    ...
    arcpy.Erase_analysis(
        in_features=prefix + "_hdwy15buf",
        erase_features=prefix + "_majorbuf",
        out_feature_class=prefix + "_hdwy15buf_only",
    )

    logger.info("Isolate {}_hdwy30buf => {}_hdwy30buf_only".format(prefix, prefix))
    arcpy.Erase_analysis(
        prefix + "_hdwy30buf", prefix + "_majorbuf", prefix + "_hdwy30buf_1"
    )
    arcpy.Erase_analysis(
        prefix + "_hdwy30buf_1", prefix + "_hdwy15buf", prefix + "_hdwy30buf_only"
    )
    ...
    arcpy.Erase_analysis(
        prefix + "_hdwy30plusbuf", prefix + "_majorbuf", prefix + "_hdwy30plusbuf_1"
    )
    arcpy.Erase_analysis(
        prefix + "_hdwy30plusbuf_1", prefix + "_hdwy15buf", prefix + "_hdwy30plusbuf_2"
    )
    arcpy.Erase_analysis(
    ...
    arcpy.Delete_management([prefix + "_hdwy30plusbuf_1", prefix + "_hdwy30plusbuf_2"])

    logger.info("Rest of Bay Area => {}_none".format(prefix))
    arcpy.Erase_analysis("BAcounty_expand", prefix + "_majorbuf", "BAcounty_expand_1")
    arcpy.Erase_analysis(
        "BAcounty_expand_1", prefix + "_hdwy15buf", "BAcounty_expand_2"
    )
    arcpy.Erase_analysis(
        "BAcounty_expand_2", prefix + "_hdwy30buf", "BAcounty_expand_3"
    )
    arcpy.Erase_analysis(
        "BAcounty_expand_3", prefix + "_hdwy30plusbuf", prefix + "_none"
    )
    arcpy.Delete_management(
        ["BAcounty_expand_1", "BAcounty_expand_2", "BAcounty_expand_3"]
    )
    ...
    with arcpy.da.UpdateCursor(
        prefix + "_cat5", ["Service_Level", "MERGE_SRC"]
    ) as cursor:
        for row in cursor:
            if "none" in row[1]:
...
if __name__ == "__main__":
    ...
    transit_layer = arcpy.MakeFeatureLayer_management(
        MTC_ONLINE_TRANSIT_URL, "transit_layer"
    )
    bacounty_portal = arcpy.MakeFeatureLayer_management(
        MTC_ONLINE_BACOUNTY_URL, "bacounty_portal"
    )
    taz_portal = arcpy.MakeFeatureLayer_management(MTC_ONLINE_TAZ_URL, "taz_portal")
    tract_portal = arcpy.MakeFeatureLayer_management(
        MTC_ONLINE_TRACT_URL, "tract_portal"
    )
    ...
    if arcpy.Exists(taz):
        arcpy.Delete_management(taz)
    if arcpy.Exists(tract):
        arcpy.Delete_management(tract)
    arcpy.FeatureClassToFeatureClass_conversion(
        bacounty_portal, arcpy.env.workspace, "bacounty"
    )
    arcpy.FeatureClassToFeatureClass_conversion(taz_portal, arcpy.env.workspace, "taz")
    arcpy.FeatureClassToFeatureClass_conversion(
        tract_portal, arcpy.env.workspace, "tract"
    )

    transit_current = arcpy.SelectLayerByAttribute_management(
        transit_layer, "NEW_SELECTION", "status = 'Existing/Built'"
    )
    arcpy.CopyFeatures_management(transit_current, "transit_current")

    transit_potential = arcpy.SelectLayerByAttribute_management(
        transit_layer, "NEW_SELECTION", "status <> 'Existing/Built'"
    )
        ...
        for model_year in (2015, 2050):
            if model_year == 2050:
                if us_runid == us_2050_FBP_Final:
                    parcel_file = urbansim_runid + "_parcel_data_{}_UBI.csv".format(
                        model_year
                    )
                else:
                    parcel_file = urbansim_runid + "_parcel_data_{}.csv".format(
                        model_year
                    )
            else:
                parcel_file = urbansim_runid + "_parcel_data_{}.csv".format(model_year)
            logger.info(
                "Reading {} parcel data from {}".format(model_year, parcel_file)
            )
            ...
            if arcpy.Exists(parcel_table):
                arcpy.management.Delete(parcel_table)
            ...
            arcpy.management.XYTableToPoint(
                in_table=parcel_table,
                out_feature_class="parcel_fc",
                x_field="x",
                y_field="y",
            )
            ...
            for transit_feature in transit_features:
                ...
                logger.info(
                    "Summarizing {} parcel data proximity to {}".format(
                ...
                arcpy.SpatialJoin_analysis(
                    target_features="parcel_fc_join_trn_taz",
                    join_features=tract,
                ...
                prox_sdf = (
                    prox_sdf_ba.groupby("Service_Level")
                    .agg(
                        {
                            "tothh": "sum",
                            "hhq1": "sum",
                ...
                prox_sdf["tothh_share"] = round(
                    prox_sdf.tothh / prox_sdf.tothh.sum(), 2
                )
                prox_sdf["hhq1_share"] = round(prox_sdf.hhq1 / prox_sdf.hhq1.sum(), 2)
                prox_sdf["totemp_share"] = round(
                    prox_sdf.totemp / prox_sdf.totemp.sum(), 2
                )
                prox_sdf["RETEMPN_share"] = round(
                    prox_sdf.RETEMPN / prox_sdf.RETEMPN.sum(), 2
                )
                prox_sdf["MWTEMPN_share"] = round(
                    prox_sdf.MWTEMPN / prox_sdf.MWTEMPN.sum(), 2
                )
                ...
                prox_sdf_coc = (
                    prox_sdf_coc.groupby("Service_Level")
                    .agg(
                        {
                            "tothh": "sum",
                            "hhq1": "sum",
                ...
                prox_sdf_coc["tothh_share"] = round(
                    prox_sdf_coc.tothh / prox_sdf_coc.tothh.sum(), 2
                )
                prox_sdf_coc["hhq1_share"] = round(
                    prox_sdf_coc.hhq1 / prox_sdf_coc.hhq1.sum(), 2
                )
                prox_sdf_coc["totemp_share"] = round(
                    prox_sdf_coc.totemp / prox_sdf_coc.totemp.sum(), 2
                )
                prox_sdf_coc["RETEMPN_share"] = round(
                    prox_sdf_coc.RETEMPN / prox_sdf_coc.RETEMPN.sum(), 2
                )
                prox_sdf_coc["MWTEMPN_share"] = round(
                    prox_sdf_coc.MWTEMPN / prox_sdf_coc.MWTEMPN.sum(), 2
                )
                ...
                prox_sdf_hra["tothh_share"] = round(
                    prox_sdf_hra.tothh / prox_sdf_hra.tothh.sum(), 2
                )
                prox_sdf_hra["hhq1_share"] = round(
                    prox_sdf_hra.hhq1 / prox_sdf_hra.hhq1.sum(), 2
                )
                prox_sdf_hra["totemp_share"] = round(
                    prox_sdf_hra.totemp / prox_sdf_hra.totemp.sum(), 2
                )
                prox_sdf_hra["RETEMPN_share"] = round(
                    prox_sdf_hra.RETEMPN / prox_sdf_hra.RETEMPN.sum(), 2
                )
                prox_sdf_hra["MWTEMPN_share"] = round(
                    prox_sdf_hra.MWTEMPN / prox_sdf_hra.MWTEMPN.sum(), 2
                )
                ...
                prox_sdf_taz = (
                    prox_sdf_ba.groupby(["area_type", "Service_Level"])
                    .agg(
                        {
                            "tothh": "sum",
                            "hhq1": "sum",
                ...
                prox_sdf_taz["tothh_share"] = round(
                    prox_sdf_taz.tothh / prox_sdf_taz.tothh.sum(), 2
                )
                prox_sdf_taz["hhq1_share"] = round(
                    prox_sdf_taz.hhq1 / prox_sdf_taz.hhq1.sum(), 2
                )
                prox_sdf_taz["totemp_share"] = round(
                    prox_sdf_taz.totemp / prox_sdf_taz.totemp.sum(), 2
                )
                prox_sdf_taz["RETEMPN_share"] = round(
                    prox_sdf_taz.RETEMPN / prox_sdf_taz.RETEMPN.sum(), 2
                )
                prox_sdf_taz["MWTEMPN_share"] = round(
                    prox_sdf_taz.MWTEMPN / prox_sdf_taz.MWTEMPN.sum(), 2
                )
