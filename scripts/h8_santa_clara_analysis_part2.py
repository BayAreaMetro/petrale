...
if __name__ == "__main__":
    ...
    if os.getenv("USERNAME") == "ywang":
        file_dir = "C:\\Users\\{}\\Documents\\ArcGIS\\Projects\\SantaClara_subsidized_blgs".format(
            os.getenv("USERNAME")
        )
        gdb_dir = "C:\\Users\\{}\\Documents\\ArcGIS\\Projects\\SantaClara_subsidized_blgs\\SantaClara_subsidized_blgs.gdb".format(
            os.getenv("USERNAME")
        )
    ...
    for filename in list(glob.glob(file_dir + "\\*.csv")):
        ...
        if "parcel_summary" in filename:
            ...
            parcel_summary_values = numpy.array(
                numpy.rec.fromrecords(parcel_summary.values)
            )
            parcel_summary_values.dtype.names = tuple(
                parcel_summary.dtypes.index.tolist()
            )
            ...
            arcpy.CopyFeatures_management("p10_scl", "p10_scl_cp")

            # join table to p10_scl layer
            print("Joining {} with {}".format("p10_scl_cp", parcel_summary_table_path))

            parcel_summary_join = arcpy.AddJoin_management(
                "p10_scl_cp",
                "PARCEL_ID",
                parcel_summary_cp,
                "parcel_id",
            ...
            arcpy.CopyFeatures_management(
                parcel_summary_join, os.path.join(gdb_dir, parcel_summary_rename)
            )
            print(
                "Completed creation of {}".format(
                    os.path.join(gdb_dir, parcel_summary_rename)
                )
            )
            field_names = [f.name for f in arcpy.ListFields(parcel_summary_rename)]
            print(
                "{} has the following fields: {}".format(
                    parcel_summary_rename, field_names
                )
            )
