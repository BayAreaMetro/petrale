...
DBP_DIR = (
    "Draft Blueprint runs/Blueprint Plus Crossing (s23)/v1.7.1- FINAL DRAFT BLUEPRINT"
)

# Add new runs here: for comparison -- using v1.8 as a placeholder for now
FBP_v3 = (
    "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.2.1 (growth summary updates)"
)
...
FBP_v7 = (
    "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.6.1 (growth summary updates)"
)
FBP_v8 = "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.7"
FBP_v9 = "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.8"
FBP_v10 = (
    "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.8.1 (parcels geography update)"
)
...
FBP_v20 = (
    "Final Blueprint runs/Final Blueprint (s24)/BAUS v2.20.1 (adds project to devproj)"
)
...
PATH_LIST = [
    ...
    FBP_v25,
    FBP_v26,
]  # ---Add new run paths to this list---
...
OUTPUT_FILE = os.path.join(URBANSIM_OUTPUT_BOX_DIR, VIZ, "PBA50_growth_{}_allruns.csv")
...
def county_calculator(DF1, DF2):
    ...
    if ("TAZ" in DF1.columns) & ("TAZ" in DF2.columns):
        ...
        DF_CO_GRWTH["TOTEMP GROWTH SHR"] = DF_CO_GRWTH["TOTEMP GROWTH"] / (
            DF_CO_GRWTH["TOTEMP_y"].sum() - DF_CO_GRWTH["TOTEMP_x"].sum()
        )
        DF_CO_GRWTH["TOTHH GROWTH SHR"] = DF_CO_GRWTH["TOTHH GROWTH"] / (
            DF_CO_GRWTH["TOTHH_y"].sum() - DF_CO_GRWTH["TOTHH_x"].sum()
        )

        DF_CO_GRWTH["TOTEMP PCT GROWTH"] = (
            DF_CO_GRWTH["TOTEMP_y"] / DF_CO_GRWTH["TOTEMP_x"] - 1
        )
        DF_CO_GRWTH["TOTHH PCT GROWTH"] = (
            DF_CO_GRWTH["TOTHH_y"] / DF_CO_GRWTH["TOTHH_x"] - 1
        )

        DF_CO_GRWTH["TOTEMP SHR CHNG"] = (
            DF_CO_GRWTH["TOTEMP_y"] / DF_CO_GRWTH["TOTEMP_y"].sum()
            - DF_CO_GRWTH["TOTEMP_x"] / DF_CO_GRWTH["TOTEMP_x"].sum()
        )
        DF_CO_GRWTH["TOTHH SHR CHNG"] = (
...
def taz_calculator(DF1, DF2):
    ...
    if ("TAZ" in DF1.columns) & ("TAZ" in DF2.columns):
        ...
        DF_merge["TOTEMP GROWTH SHR"] = DF_merge["TOTEMP GROWTH"] / (
            DF_merge["TOTEMP_y"].sum() / DF_merge["TOTEMP_x"].sum()
        )
        DF_merge["TOTHH GROWTH SHR"] = DF_merge["TOTHH GROWTH"] / (
        ...
        DF_TAZ_GROWTH = DF_TAZ_GROWTH.rename(
            columns={"SD_x": "SD", "ZONE_x": "ZONE", "COUNTY_x": "COUNTY"}
        )
...
def FILELOADER(path, geo, baseyear, endyear):
    ...
    # DBP has a different pattern from all new BP runs
        ...
        localpath = os.path.join(
            URBANSIM_OUTPUT_BOX_DIR, path
        )  # the path is the run folder, must be specificed.
        pattern_baseyear = "(run[0-9]{1,4}_%s_summaries_%s.csv)" % (geo, baseyear)
        pattern_endyear = "(run[0-9]{1,4}_%s_summaries_%s.csv)" % (geo, endyear)
...
def GEO_SUMMARY_LOADER(path, geo, baseyear, endyear):
    ...
    parcel_baseyear_search = [
        f for f in os.listdir(localpath) if re.match(parcel_baseyear_pattern, f)
    ]
    ...
    parcel_endyear_search = [
        f for f in os.listdir(localpath) if re.match(parcel_endyear_pattern, f)
    ]
    ...
    parcel_geobase_file = pd.read_csv(
        os.path.join(URBANSIM_OUTPUT_BOX_DIR, PARCEL_GEO_DIR)
    )
    parcel_geobase_file_r = parcel_geobase_file[["PARCEL_ID", "juris", "fbpchcat"]]
    parcel_baseyear = parcel_baseyear[
        ["parcel_id", "tothh", "totemp", "hhq1", "hhq2", "hhq3", "hhq4"]
    ]
    parcel_endyear = parcel_endyear[
        ["parcel_id", "tothh", "totemp", "hhq1", "hhq2", "hhq3", "hhq4"]
    ]
    parcel_merge = parcel_baseyear.merge(parcel_endyear, on="parcel_id").fillna(0)
    parcel_data = parcel_geobase_file_r.merge(
        parcel_merge, left_on="PARCEL_ID", right_on="parcel_id", how="left"
    )
    # else:
    ...
    parcel_geo_no = parcel_data.loc[
        ~parcel_data["fbpchcat"].str.contains(geo, na=False)
    ]
    parcel_geo_no = (
        parcel_geo_no.groupby(["juris"])
        .agg(
            {
                "tothh diff": "sum",
                "totemp diff": "sum",
...
def TWO_GEO_SUMMARY_LOADER(path, geo1, geo2, baseyear, endyear):
    ...
    parcel_geo2_no_1 = parcel_data.loc[
        parcel_data["fbpchcat"].str.contains(geo1, na=False)
    ]
    parcel_geo2_no_1 = parcel_geo2_no_1.loc[
        ~parcel_geo2_no_1["fbpchcat"].str.contains(geo2, na=False)
    ]
    parcel_geo2_no_2 = parcel_data.loc[
        parcel_data["fbpchcat"].str.contains(geo2, na=False)
    ]
    parcel_geo2_no_2 = parcel_geo2_no_2.loc[
        ~parcel_geo2_no_2["fbpchcat"].str.contains(geo1, na=False)
    ]
    parcel_geo2_no_3 = parcel_data.loc[
        ~parcel_data["fbpchcat"].str.contains(geo1 + "|" + geo2, na=False)
    ]
...
if __name__ == "__main__":
    ...
    for path in PATH_LIST:
        ...
        if summary_runid == "run7224c":
            ...
            new_names = [
                (i, summary_runid + "_" + i) for i in DF.iloc[:, 1:].columns.values
            ]
            DF.rename(columns=dict(new_names), inplace=True)
        ...
        if summary_runid == "run7224c":
            new_names = [
                (i, "PBA40_" + i) for i in DF_COUNTY.iloc[:, 1:].columns.values
            ]
            ...
            new_names = [
                (i, summary_runid + "_" + i)
                for i in DF_COUNTY.iloc[:, 1:].columns.values
            ]
            DF_COUNTY.rename(columns=dict(new_names), inplace=True)
    ...
    DF_MERGE = reduce(
        lambda left, right: pd.merge(left, right, on="TAZ", how="outer"), DF_LIST
    )
    DF_MERGE.to_csv(OUTPUT_FILE.format(GEO), index=False)

    DF_COUNTY_MERGE = reduce(
    ...
    for geo in GEO:
        ...
        DF_MERGE = reduce(
            lambda left, right: pd.merge(left, right, on=geo, how="outer"), DF_LIST
        )
        DF_MERGE.to_csv(OUTPUT_FILE.format(geo), index=False)
        DF_LIST = []
    ...
    for geo in GEO:
        ...
        DF_UNION.sort_values(
            by=["VERSION", "county", "juris", "geo_category"],
