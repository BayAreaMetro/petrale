...
if __name__ == "__main__":
    ...
    for geo in GEO:
        ...
        DF_MERGE = reduce(lambda left,right: pd.merge(left, right, on = ['juris','geo_category'], how='outer'), DF_LIST)
        DF_MERGE['county'] = DF_MERGE['juris'].map(juris_to_county)
        DF_MERGE.sort_values(by = ['county','juris','geo_category'],ascending=[True, True, False], inplace=True)
        DF_MERGE.set_index(['county','juris','geo_category'], inplace=True)
        DF_MERGE.to_csv(OUTPUT_FILE.format(geo))
        DF_LIST = []
    ...
    DF_MERGE_2 = reduce(lambda left,right: pd.merge(left, right, on = ['juris','geo_category'], how='outer'), DF_LIST_2)
    DF_MERGE_2['county'] = DF_MERGE_2['juris'].map(juris_to_county)
    DF_MERGE_2.sort_values(by = ['county','juris','geo_category'],ascending=[True, True, False], inplace=True)
    DF_MERGE_2.set_index(['county','juris','geo_category'], inplace=True)
